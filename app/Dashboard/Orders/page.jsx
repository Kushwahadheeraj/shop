    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Name, email or phone" className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={preset} onChange={(e)=>setPreset(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div className="flex items-end">
            <button onClick={fetchOrders} className="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Refresh</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  orders.map((o) => (
                    <tr key={o._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        ...
                        <div className="text-xs text-gray-500">{getCustomerPhone(o)}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button className="text-blue-600 hover:text-blue-800 text-sm" onClick={()=>openOrder(o._id)}>View</button>
                      </td>
                    </tr>
                  ))
                )}
        ...
        <div className="bg-white rounded-lg shadow p-4">
          {!selected ? (
            <div className="text-gray-500 text-center py-16">Select an order to view details</div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-lg font-semibold">Order #{selected._id?.slice(-6)}</div>
                  <div className="text-xs text-gray-500">Placed on {new Date(selected.createdAt).toLocaleString()}</div>
                </div>
                <button onClick={printInvoice} className="px-3 py-2 bg-gray-800 text-white rounded-md text-sm">Print Invoice</button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border rounded-md p-3">
                  ...
                  <div>{getCustomerName(selected)}</div>
                  <div className="text-sm text-gray-600">{[selected.address?.street, selected.address?.city, selected.address?.state, selected.address?.pin].filter(Boolean).join(', ')}</div>
                  <div className="text-sm text-gray-600">{getCustomerPhone(selected)}{getCustomerEmail(selected) ? ` • ${getCustomerEmail(selected)}` : ''}</div>
                </div>
                <div className="border rounded-md p-3">
                  ...
                  <tbody>
                    {selected.items?.map((it, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="p-2 border">₹{(it.price ?? 0).toFixed(2)}</td>
                        <td className="p-2 border">₹{((it.price ?? 0) * (it.quantity ?? 1)).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {/* Tracking update */}
              <div className="border rounded-md p-3">
                <div className="font-medium mb-2">Tracking Updates</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-3">
                  <select id="st" className="px-2 py-2 border rounded" defaultValue="">
                  ...
                  <input id="note" placeholder="Optional note" className="px-2 py-2 border rounded md:col-span-2" />
                  <button
                    onClick={() => {
