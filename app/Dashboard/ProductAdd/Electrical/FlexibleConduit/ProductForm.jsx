...
export default function ProductForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/electrical/flexible-conduit/create`, { 
        method: 'POST', 
        body: data 
      });
      ...
      if (res.ok) {
        ...
        setForm({ name: '', category: 'FlexibleConduit' });
        setFile(null);
        setPreview(null);
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md mx-auto space-y-6 p-8 bg-white rounded-xl shadow-lg border border-gray-200">
      <h2 className="text-2xl font-bold mb-6 text-center">Add FlexibleConduit Product</h2>
