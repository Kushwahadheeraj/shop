...
const tagsList = [" K I T K A T Fuses"];
...
export default function ProductForm() {
  ...
  const [categoryValue] = useState(" K I T K A T Fuses");
  ...
  const handleSubmit = async e => {
    ...
    // const res = await fetch("/api/electrical/kitkatfuses/create", { method: "POST", body: formData });
    // if (res.ok) { ... }
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl mx-auto p-4 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-4 text-center">Add  K I T K A T Fuses Product</h2>
