...
const tagsList = [" Switch And Socket"];
...
export default function ProductForm() {
  ...
  const [categoryValue] = useState(" Switch And Socket");
  ...
  const handleSubmit = async e => {
    ...
    // const res = await fetch("/api/electrical/switchandsocket/create", { method: "POST", body: formData });
    // if (res.ok) { ... }
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl mx-auto p-4 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-4 text-center">Add  Switch And Socket Product</h2>
