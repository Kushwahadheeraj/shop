...
//     try {
//       const res = await fetch(`${API_BASE_URL}/home/productTools/create`, { 
//         method: 'POST', 
//         body: data 
//       });
      
//       if (res.ok) {
//         alert('Product tool created successfully!');
//         setForm({
//           name: '',
//           description: '',
//           category: '',
...
//         alert(errorData.message || 'Error creating product tool');
//       }
//     } catch (error) {
//       console.error('Error:', error);
//       alert('Error creating product tool');
//     }
//   };
...
export default function ProductForm() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      ...
      setError("");
      try {
        // Use correct producttools home endpoint
        const res = await fetch(`${API_BASE_URL}/home/producttools/get?firstPerCategory=true`);
        const json = await res.json();
        const rows = Array.isArray(json?.data) ? json.data : Array.isArray(json) ? json : [];
        const cats = rows.map((r) => (r.category || 'Uncategorized')).filter(Boolean).sort((a, b) => a.localeCompare(b));
  ...
  const handleSubmit = (e) => {
    ...
    // Persist the selection into Home ProductTools selection
    fetch(`${API_BASE_URL}/home/producttools/select-categories`, {
      ...
      router.push(`/Dashboard/ProductList/Home/ProductTools?${qs}`);
    }).catch(() => {
      const qs = `categories=${encodeURIComponent(JSON.stringify(selectedCategories))}`;
      router.push(`/Dashboard/ProductList/Home/ProductTools?${qs}`);
    });
