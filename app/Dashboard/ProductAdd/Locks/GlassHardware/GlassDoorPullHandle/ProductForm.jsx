...
export default function ProductForm() {
  return (<h2 className="text-xl font-bold mb-2">Add GlassDoorPullHandle Product</h2>)
  const pathname = usePathname();
  const pathParts = pathname.split("/").filter(Boolean);
  const resource = pathParts[pathParts.length - 1].toLowerCase();
  const apiUrl = ${API_BASE_URL}/glasshardware//create;
  const [form, setForm] = useState({
    ...
    price: "",
    totalProduct: "",
    category: "Glass Door Pull Handle",
    tag: "",
    photos: [],
  });
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch(API_BASE_URL + "/electrical/Glass Door Pull Handle", {
        method: "POST",
        body: data,
      });
