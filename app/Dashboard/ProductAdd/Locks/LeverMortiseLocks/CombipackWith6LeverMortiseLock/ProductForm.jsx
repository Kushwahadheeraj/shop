...
export default function ProductForm() {
  return (<h2 className="text-xl font-bold mb-2">Add CombipackWith6LeverMortiseLock Product</h2>)
  const pathname = usePathname();
  const pathParts = pathname.split("/").filter(Boolean);
  const resource = pathParts[pathParts.length - 1].toLowerCase();
  const apiUrl = ${API_BASE_URL}/levermortiselocks//create;
  const [form, setForm] = useState({
    ...
    price: "",
    totalProduct: "",
    category: "Combipack With6 Lever Mortise Lock",
    tag: "",
    photos: [],
  });
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch(API_BASE_URL + "/electrical/Combipack With6 Lever Mortise Lock", {
        method: "POST",
        body: data,
      });
