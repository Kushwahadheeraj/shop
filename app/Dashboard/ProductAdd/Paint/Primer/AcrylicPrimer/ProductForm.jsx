...
export default function ProductForm() {
  return (<h2 className="text-xl font-bold mb-2">Add AcrylicPrimer Product</h2>)
  const pathname = usePathname();
  const pathParts = pathname.split("/").filter(Boolean);
  const resource = pathParts[pathParts.length - 1].toLowerCase();
  const apiUrl = ${API_BASE_URL}/primer//create;
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch(API_BASE_URL + "/electrical/Acrylic Primer", {
        method: "POST",
        body: data,
      });
