...
export default function ProductForm() {
  return (<h2 className="text-xl font-bold mb-2">Add NewSuperGlowSeries Product</h2>)
  const pathname = usePathname();
  const pathParts = pathname.split("/").filter(Boolean);
  const resource = pathParts[pathParts.length - 1].toLowerCase();
  const apiUrl = ${API_BASE_URL}/coralbathfixtures//create;
  const [form, setForm] = useState({
    ...
    price: "",
    totalProduct: "",
    category: "New Super Glow Series",
    tag: "",
    photos: [],
  });
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch(API_BASE_URL + "/electrical/New Super Glow Series", {
        method: "POST",
        body: data,
      });
