...
export default function ProductForm() {
  return (<h2 className="text-xl font-bold mb-2">Add European Water Closet Product</h2>)
  const pathname = usePathname();
  const pathParts = pathname.split("/").filter(Boolean);
  const resource = pathParts[pathParts.length - 1].toLowerCase();
  const apiUrl = ${API_BASE_URL}/europeanwatercloset//create;
  const [form, setForm] = useState({
    ...
    price: "",
    totalProduct: "",
    category: "European Water Closet",
    tag: "",
    photos: [],
  });
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch(API_BASE_URL + "/electrical/European Water Closet", {
        method: "POST",
        body: data,
      });
