    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-lg p-4 sm:p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Add Adaptors Product</h1>
        </div>
    ...
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-4 sm:space-y-6 p-4 sm:p-8 bg-white rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <Button type="button" onClick={handleAddAmp} className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1">Add Amp</Button>
        </div>
        {form.amps.map((a, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
            <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" placeholder="Amp Value" value={a.amps} onChange={e => handleAmpChange(idx, 'amps', e.target.value)} />
            <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" type="number" placeholder="Price" value={a.price} onChange={e => handleAmpChange(idx, 'price', e.target.value)} />
            <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" type="number" placeholder="Discounted Price (auto)" value={a.discountPrice} readOnly />
            <Button type="button" onClick={() => handleRemoveAmp(idx)} className="bg-red-500 hover:bg-red-600 text-white px-2 sm:px-3 py-1 text-xs sm:text-sm w-full sm:w-auto">Remove</Button>
          </div>
        ))}
      </div>
      {/* Description */}
