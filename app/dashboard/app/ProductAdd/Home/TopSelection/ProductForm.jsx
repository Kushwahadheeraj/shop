...
export default function ProductForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/home/topselection/create`, { 
        method: 'POST', 
        body: data 
      });
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-lg p-4 sm:p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Add Top Selection Item</h1>
        </div>
        <p className="text-xs sm:text-sm text-blue-50">Fill in the item details below</p>
      </div>
    ...
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-4 sm:space-y-6 p-4 sm:p-8 bg-white rounded-xl shadow-lg border border-gray-200">
      ...
      <div>
        ...
        required 
        className="w-full focus:ring-blue-300 focus:border-blue-500 text-sm sm:text-base"
        />
      </div>
      ...
      <div>
        ...
        value={form.tag} 
        onChange={handleChange} 
        placeholder="Enter tag (e.g. Best Seller)" 
        className="w-full focus:ring-blue-300 focus:border-blue-500 text-sm sm:text-base"
        />
      </div>
      ...
      <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white">Create Item</Button>
    </form>
    </div>
  );
}
