...
export default function ProductForm({ onSave }) {
  ...
  // Predefined color options imported from shared config imported from shared config
  ...
  const handleSubmit = async e => {
    ...
    customFields.forEach((f, idx) => {
      if (f.fieldName) {
        ...
        f.fieldValues.forEach(val => {
          if (val) data.append('customFieldValue' + (idx+1), val);
        });
      }
    });
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/paint/acrylic-emulsion-paint/create`, { method: 'POST', body: data });
      if (res.ok) {
        onSave && onSave();
      } else {
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-lg p-4 sm:p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Add AcrylicEmulsionPaint Product</h1>
        </div>
    ...
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-4 sm:space-y-6 p-4 sm:p-8 bg-white rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2 mb-2">
          ...
          />
          <Button type="button" onClick={handleAddColor} className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1">Add</Button>
        </div>
