...
export default function ProductForm({ onSave }) {
  ...
  const [form, setForm] = useState({
    ...
    discountPrice: '',
    totalProduct: '',
    category: 'VarnishBlackBoardPaint',
    description: '',
    tags: [],
    colors: [],
  ...
  const handleChange = e => {
    ...
    setForm(prev => {
      ...
      if (name === 'price' || name === 'discount') {
        const price = parseFloat(name === 'price' ? value : prev.fixPrice);
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/paint/wood-finishes-varnish-black-board-paint/create`, { method: 'POST', body: data });
    if (res.ok) {
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-lg p-4 sm:p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Add VarnishBlackBoardPaint Product</h1>
        </div>
