...
export default function ProductForm({ onSave }) {
  ...
  // Calculate price range from type options
  const calculatePriceRange = () => {
    let minPrice = Infinity;
    let maxPrice = -Infinity;
    
    customFields.forEach(field => {
      if (field.fieldName && field.fieldValues.length > 0) {
  ...
  const handleCustomFieldNameChange = (idx, value) => {
    ...
    // Auto-calculate price range after a short delay
  ...
  const handleAddVariant = () => {
    setForm(prev => ({ ...prev, variants: [...prev.variants, { variantName: '', fixPrice: '', minPrice: '', maxPrice: '', discountPrice: '' }] }));
  };
  ...
  const handleSubmit = async e => {
    ...
    // Add type array from custom fields
    const typeArray = [];
    customFields.forEach(field => {
      if (field.fieldName && field.fieldValues.length > 0) {
        field.fieldValues.forEach(value => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/pipe/ashirvad-pipes/create`, { 
        method: 'POST', 
        body: data 
      });
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-lg p-4 sm:p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Add AshirvadPipes Product</h1>
        </div>
    ...
    <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-4 sm:space-y-6 p-4 sm:p-8 bg-white rounded-xl shadow-lg border border-gray-200">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <Button type="button" onClick={calculatePriceRange} className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1">
        ...
        {customFields.map((f, idx) => (
          <div key={idx} className="mb-2 border p-2 rounded">
            <div className="flex gap-2 items-center mb-1">
              <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" placeholder="Fit Name" value={f.fieldName} onChange={e => handleCustomFieldNameChange(idx, e.target.value)} />
            </div>
            {f.fieldValues.map((val, vIdx) => (
              <div key={vIdx} className="flex gap-2 items-center mb-1">
                <Input className="w-1/2 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" placeholder="Rate (Price)" value={val} onChange={e => handleCustomFieldValueChange(idx, vIdx, e.target.value)} />
            ...
            <Button type="button" onClick={() => handleAddCustomFieldValue(idx)} className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1">Add Rate</Button>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-2">
        ...
        {form.variants.map((v, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
            ...
            <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" type="number" placeholder="Fix Price" value={v.fixPrice} onChange={e => handleVariantChange(idx, 'fixPrice', e.target.value)} />
