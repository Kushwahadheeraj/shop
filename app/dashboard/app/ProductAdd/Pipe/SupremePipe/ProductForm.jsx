...
export default function ProductForm({ onSave }) {
  ...
  const handleSubmit = async e => {
    ...
    Object.entries(form).forEach(([k, v]) => {
      ...
      // Skip variants, fixPrice, totalProduct, discount as they're handled separately
    });
    ...
    const res = await fetch(`${API_BASE_URL}/pipe/supreme-pipes/create`, { method: 'POST', body: data });
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6 bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 rounded-lg p-4 sm:p-6 text-white shadow-lg">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Add SupremePipes Product</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
        ...
        <div className="col-span-2">
          <Button 
            ...
            className="bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1"
          >
        ...
        {customFields.map((f, idx) => (
          <div key={idx} className="mb-2 border p-2 rounded">
            <div className="flex gap-2 items-center mb-1">
              <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" placeholder="Fit Name (e.g., 20mm, 25mm)" value={f.fieldName} onChange={e => handleCustomFieldNameChange(idx, e.target.value)} />
            </div>
            {f.fieldValues.map((val, vIdx) => (
              <div key={vIdx} className="flex gap-2 items-center mb-1">
                <Input className="w-1/2 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" type="number" placeholder="Rate (Price)" value={val} onChange={e => handleCustomFieldValueChange(idx, vIdx, e.target.value)} />
      ...
      <div className="space-y-2">
        ...
        {form.variants.map((v, idx) => (
          <div key={idx} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
            ...
            <Input className="w-1/3 focus:ring-amber-300 focus:border-amber-500 text-sm sm:text-base" type="number" placeholder="Price" value={v.fixPrice} onChange={e => handleVariantChange(idx, 'fixPrice', e.target.value)} />
