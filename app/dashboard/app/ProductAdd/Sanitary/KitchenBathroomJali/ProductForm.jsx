...
export default function ProductForm({ onSave }) {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/sanitary/kitchen-bathroom-jali/getOne/${id}`);
      ...
      setForm({
        ...
        category: data.category || 'KitchenBathroomJali',
  ...
  const handleSubmit = async e => {
    ...
    try {
      const url = isEditMode 
        ? `${API_BASE_URL}/sanitary/kitchen-bathroom-jali/update/${id}`
        : `${API_BASE_URL}/sanitary/kitchen-bathroom-jali/create`;
  ...
  return (
    <form onSubmit={handleSubmit} className="p-8 bg-white rounded-lg shadow-md max-w-4xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
        ...
        Add New Product (Kitchen/Bathroom Jali)
      </h2>
