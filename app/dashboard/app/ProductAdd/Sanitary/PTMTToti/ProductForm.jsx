...
export default function ProductForm({ onSave }) {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/sanitary/ptmt-toti/getOne/${id}`);
      ...
      setForm({
        ...
        category: data.category || 'PTMTToti',
      ...
      if (data.customFields && Array.isArray(data.customFields)) {
          ...
          // Ensure at least 3 fields structure if needed, or just set what we have
          // For now let's just set what we have and pad if less than 3
  ...
  const handleSubmit = async e => {
    ...
    try {
      const url = isEditMode 
        ? `${API_BASE_URL}/sanitary/ptmt-toti/update/${id}`
        : `${API_BASE_URL}/sanitary/ptmt-toti/create`;
