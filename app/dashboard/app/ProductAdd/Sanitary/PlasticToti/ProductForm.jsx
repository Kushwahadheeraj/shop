...
export default function ProductForm({ onSave }) {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/sanitary/plastic-toti/getOne/${id}`);
      ...
      if (data.customFields && Array.isArray(data.customFields)) {
         const mappedFields = data.customFields.map(cf => ({
            fieldName: cf.fieldName,
            fieldValues: cf.fieldValues && cf.fieldValues.length > 0 ? cf.fieldValues : ['']
         }));
         // Fill up to 3 if less
         while (mappedFields.length < 3) {
            mappedFields.push({ fieldName: '', fieldValues: [''] });
         }
  ...
  const handleSubmit = async e => {
    ...
    try {
      const url = isEditMode 
        ? `${API_BASE_URL}/sanitary/plastic-toti/update/${id}`
        : `${API_BASE_URL}/sanitary/plastic-toti/create`;
