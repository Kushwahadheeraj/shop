...
export default function ProductForm({ onSave }) {
  ...
  const fetchProduct = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/sanitary/sink-wash-basin-jali/getOne/${id}`);
      ...
      setForm({
        ...
        category: data.category || 'SinkWashBasinJali',
  ...
  const handleSubmit = async e => {
    ...
    try {
      const url = isEditMode 
        ? `${API_BASE_URL}/sanitary/sink-wash-basin-jali/update/${id}`
        : `${API_BASE_URL}/sanitary/sink-wash-basin-jali/create`;
  ...
  return (
    <form onSubmit={handleSubmit} className="p-8 bg-white rounded-lg shadow-md max-w-4xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 flex items-center gap-2">
        ...
        Add New Product (Sink/Wash Basin Jali)
      </h2>
