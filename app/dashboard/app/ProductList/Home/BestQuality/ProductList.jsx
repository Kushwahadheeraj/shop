...
export default function ProductList() {
  ...
  const API_URL = `${API_BASE_URL}/home/bestquality`;
  ...
  const handleAddNew = () => {
    router.push("/dashboard/app/ProductAdd/Home/BestQuality");
  };
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Best Quality Items</h1>
            <p className="text-sm text-gray-500">Manage your Best Quality section items</p>
          </div>
        </div>
              ...
              {loading ? (
                ...
                filteredItems.map((item) => (
                  <TableRow key={item._id} className="hover:bg-gray-50/50">
                    ...
                    <TableCell>
                      {item.tag && <Badge variant="secondary" className="bg-emerald-50 text-emerald-700">{item.tag}</Badge>}
                    </TableCell>
