    ...
    try {
      const res = await fetch(`${API_BASE_URL}/payments/razorpay/order`, {
        ...
        body: JSON.stringify({
          amount: amountPaise,
          currency: 'INR',
          receipt: pending?.clientOrderId || `co_${Date.now()}`,
          notes: { source: 'shop', userId: pending?.userId || '' },
        }),
      });
      const json = await res.json();
      if (!json?.success) throw new Error(json?.message || 'Failed to initialize payment');
      const key = json?.data?.key || process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || '';
      const options = {
        key,
        ...
        prefill: {
          name: `${pending?.address?.firstName || ''} ${pending?.address?.lastName || ''}`.trim(),
          email: pending?.address?.email || '',
          contact: pending?.address?.phone || '',
        },
        notes: {
