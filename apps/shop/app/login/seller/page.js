...
function SellerLoginPageContent() {
  ...
  const checkMobileExists = async (mobile) => {
    ...
    setMobileError("");
    try {
      const res = await fetch(`${API_BASE_URL}/seller/check-mobile?mobile=${mobile}`);
      const data = await res.json();
      if (data.exists) {
        setMobileError(`This mobile number is already registered by another seller`);
      ...
      // If API doesn't exist, we'll handle it in registration
      setMobileError("");
      setExistingMobile("");
    } finally {
  ...
  const handleChange = (e) => {
    ...
    // Clear mobile error when user changes the number
    if (name === "mobile") {
      if (value !== existingMobile) {
        setMobileError("");
        setExistingMobile("");
      }
      ...
      if (value.length === 10 && isValidMobile(value)) {
        mobileCheckTimeoutRef.current = setTimeout(() => checkMobileExists(value), 500);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "login") {
        ...
        const data = await res.json();
        if (!res.ok) {
          // Check if error is about mobile number
          const errorMsg = data.message || "Something went wrong";
          if (errorMsg.toLowerCase().includes("mobile") || errorMsg.toLowerCase().includes("number")) {
            setMobileError(errorMsg);
            setExistingMobile(form.mobile);
            ...
            <>
              ...
              <div>
               ...
               <div>
                 <div className="relative">
                   <input
                     ...
                     className={`w-full border rounded px-3 py-2 pr-10 ${
                       mobileError || (form.mobile && !isValidMobile(form.mobile)) ? "border-red-500" : ""
                     }`}
                     required
                     maxLength={10}
                   />
                 ...
                 {form.mobile && !isValidMobile(form.mobile) && !mobileError && (
                   <p className="text-red-500 text-xs mt-1">Please enter a valid 10-digit mobile number</p>
                 )}
                 {mobileError && (
                   <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded">
                     <p className="text-red-600 text-xs font-semibold mb-1">{mobileError}</p>
                     {existingMobile && (
                       <div className="flex items-center justify-between mt-2">
                         <p className="text-xs text-gray-600">
                           Existing number: <span className="font-semibold text-gray-800">{existingMobile}</span>
                         </p>
                         <button
                           type="button"
                           onClick={() => {
