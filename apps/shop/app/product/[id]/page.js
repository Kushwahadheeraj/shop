...
export default function ProductDetailPage() {
  ...
  const priceBlock = useMemo(() => {
    ...
    if (optionCurrent != null || optionOriginal != null) {
      return (
        <div className="flex items-center gap-3">
          <span className="font-bold text-2xl">₹{Number(optionCurrent ?? 0).toLocaleString('en-IN')}</span>
        </div>
      );
    }
    ...
    if (hasRange) {
      return (
        <div className="flex items-center gap-3">
          <span className="font-bold text-2xl">₹{Number(product.minPrice).toLocaleString('en-IN')} – ₹{Number(product.maxPrice).toLocaleString('en-IN')}</span>
        </div>
      );
    }
    ...
    return (
      <div className="flex items-center gap-3">
        <span className="font-bold text-2xl">₹{Number(current || 0).toLocaleString('en-IN')}</span>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mt-36 mx-auto p-4 sm:p-6">
      <div className="text-sm text-gray-500 mb-4">
        <Link href="/">Home</Link> / <span className="capitalize">{product.type || 'Product'}</span>
      </div>
      <div className="flex flex-col lg:flex-row gap-6">
        <div className="lg:w-[42%] bg-white rounded border p-0">
          <div className="relative w-full h-[480px] overflow-hidden flex items-center justify-center">
            <button
              ...
              className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 border rounded-full w-8 h-8 flex items-center justify-center shadow"
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 border rounded-full w-8 h-8 flex items-center justify-center shadow"
        ...
        <div className="lg:flex-1 space-y-6">
          ...
          {Array.isArray(product.variants) && product.variants.length > 0 && (
            <div className="space-y-2 mt-3">
              ...
              <select value={variant} onChange={(e)=>setVariant(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm">
          ...
          {Array.isArray(product.typeOptions) && product.typeOptions.length > 0 && (
            <div className="space-y-2 mt-3">
              ...
              <select value={typeOption} onChange={(e)=>setTypeOption(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm">
          ...
          {Array.isArray(product.brand) && product.brand.length > 0 && (
            <div className="space-y-2 mt-3">
              ...
              <select value={brandSel} onChange={(e)=>setBrandSel(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm">
          ...
          {Array.isArray(product.way) && product.way.length > 0 && (
            <div className="space-y-2 mt-3">
              <div className="text-sm font-medium text-gray-700">WAY</div>
              <select value={waySel} onChange={(e)=>setWaySel(e.target.value)} className="w-full border border-gray-300 rounded px-3 py-2 text-sm">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <button
                className="bg-black hover:bg-gray-800 text-white font-semibold px-8 py-3 rounded text-sm"
                onClick={() => {
      ...
      <div className="mt-8 border-t pt-6">
        <div className="flex items-center gap-6 text-sm font-medium">
          <button onClick={()=>setActiveTab('info')} className={`${activeTab==='info' ? 'border-b-2 border-black' : 'text-gray-400'} pb-2`}>ADDITIONAL INFORMATION</button>
          <button onClick={()=>setActiveTab('reviews')} className={`${activeTab==='reviews' ? 'border-b-2 border-black' : 'text-gray-400'} pb-2`}>REVIEWS ({reviews.length})</button>
        </div>
        {activeTab==='info' && (
          <div className="mt-4">
            ...
            {(() => {
              ...
              return rows.length > 0 ? (
                <div className="divide-y border rounded">
                  {rows.map(([k,v], i) => (
                    <div key={i} className="flex justify-between items-start gap-4 p-3 text-sm">
                      <span className="font-medium text-gray-700">{k}</span>
                      <span className="text-gray-800 text-right flex-1">{v}</span>
                    </div>
                  ))}
                </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <h4 className="text-base font-semibold mb-3">{reviews.length} reviews for {product.name}</h4>
          <div className="space-y-4">
            {reviews.map((r) => (
        ...
        <div>
          <div className="border p-4 rounded">
            <h5 className="font-semibold mb-3">Add a review</h5>
            ...
            {(!hasToken && !user) && (
              <div className="text-xs text-red-600 mb-2">Please login to write a review.</div>
            )}
      ...
      {related.length > 0 && (
        <div className="mt-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {related.map((r, idx) => (
              <div key={`${r.id}-${idx}`} className="group bg-white rounded border p-3 flex flex-col hover:shadow-md transition">
                {r.discount > 0 && (
                ...
                <div className="mt-auto">
                  {r.minPrice != null && r.maxPrice != null && Number(r.minPrice) > 0 && Number(r.maxPrice) > 0 && Number(r.minPrice) !== Number(r.maxPrice) ? (
                    ...
                    <>
                      {r.price != null && r.discount > 0 && (
                        <span className="text-gray-400 line-through text-xs mr-2">₹{Number(r.price).toLocaleString('en-IN')}</span>
                      )}
                      <span className="font-semibold">₹{Number((r.discountPrice ?? r.price) || 0).toLocaleString('en-IN')}</span>
                    </>
                  )}
                </div>
                <div className="pt-2">
