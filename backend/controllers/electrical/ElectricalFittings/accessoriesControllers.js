...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Accessories] Create request');
      console.log('[Accessories] Body:', req.body);
      console.log('[Accessories] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Accessories] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Accessories] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Accessories] Raw amps:', amps);
      console.log('[Accessories] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Accessories'
    };

    if (shouldLog) console.log('[Accessories] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Accessories] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createAccessories:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Accessories' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Accessories' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Accessories] Update id:', req.params.id);
      console.log('[Accessories] Body:', req.body);
      console.log('[Accessories] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Accessories' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Accessories' });
    if (!product) return res.status(404).json({ error: 'Not found' });
