...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CircularDeepBox] Create request');
      console.log('[CircularDeepBox] Body:', req.body);
      console.log('[CircularDeepBox] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[CircularDeepBox] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[CircularDeepBox] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[CircularDeepBox] Raw amps:', amps);
      console.log('[CircularDeepBox] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'CircularDeepBox'
    };

    if (shouldLog) console.log('[CircularDeepBox] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[CircularDeepBox] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createCircularDeepBox:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Circulardeepbox' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Circulardeepbox' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CircularDeepBox] Update id:', req.params.id);
      console.log('[CircularDeepBox] Body:', req.body);
      console.log('[CircularDeepBox] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Circulardeepbox' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateCircularDeepBox:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Circulardeepbox' });
    if (!product) return res.status(404).json({ error: 'Not found' });
