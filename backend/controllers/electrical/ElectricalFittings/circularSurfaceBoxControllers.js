...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CircularSurfaceBox] Create request');
      console.log('[CircularSurfaceBox] Body:', req.body);
      console.log('[CircularSurfaceBox] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[CircularSurfaceBox] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[CircularSurfaceBox] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[CircularSurfaceBox] Raw amps:', amps);
      console.log('[CircularSurfaceBox] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'CircularSurfaceBox'
    };

    if (shouldLog) console.log('[CircularSurfaceBox] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[CircularSurfaceBox] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createCircularSurfaceBox:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'circularsurfacebox' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'circularsurfacebox' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CircularSurfaceBox] Update id:', req.params.id);
      console.log('[CircularSurfaceBox] Body:', req.body);
      console.log('[CircularSurfaceBox] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'circularsurfacebox' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateCircularSurfaceBox:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'circularsurfacebox' });
    if (!product) return res.status(404).json({ error: 'Not found' });
