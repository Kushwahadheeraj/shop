...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[RigidType] Create request');
      console.log('[RigidType] Body:', req.body);
      console.log('[RigidType] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[RigidType] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[RigidType] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[RigidType] Raw amps:', amps);
      console.log('[RigidType] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'RigidType'
    };

    if (shouldLog) console.log('[RigidType] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[RigidType] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createRigidType:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'rigidtype' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'rigidtype' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[RigidType] Update id:', req.params.id);
      console.log('[RigidType] Body:', req.body);
      console.log('[RigidType] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'rigidtype' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateRigidType:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'rigidtype' });
    if (!product) return res.status(404).json({ error: 'Not found' });
