...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CabinFans] Create request');
      console.log('[CabinFans] Body:', req.body);
      console.log('[CabinFans] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[CabinFans] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[CabinFans] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[CabinFans] Raw amps:', amps);
      console.log('[CabinFans] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'CabinFans'
    };

    if (shouldLog) console.log('[CabinFans] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[CabinFans] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createCabinFans:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'cabinfans' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'cabinfans' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CabinFans] Update id:', req.params.id);
      console.log('[CabinFans] Body:', req.body);
      console.log('[CabinFans] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'cabinfans' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateCabinFans:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'cabinfans' });
    if (!product) return res.status(404).json({ error: 'Not found' });
