...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CeilingFans] Create request');
      console.log('[CeilingFans] Body:', req.body);
      console.log('[CeilingFans] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[CeilingFans] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[CeilingFans] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[CeilingFans] Raw amps:', amps);
      console.log('[CeilingFans] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'CeilingFans'
    };

    if (shouldLog) console.log('[CeilingFans] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[CeilingFans] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createCeilingFans:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Ceilingfans' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Ceilingfans' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CeilingFans] Update id:', req.params.id);
      console.log('[CeilingFans] Body:', req.body);
      console.log('[CeilingFans] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Ceilingfans' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateCeilingFans:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Ceilingfans' });
    if (!product) return res.status(404).json({ error: 'Not found' });
