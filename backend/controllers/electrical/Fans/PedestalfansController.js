...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Pedestalfans] Create request');
      console.log('[Pedestalfans] Body:', req.body);
      console.log('[Pedestalfans] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Pedestalfans] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Pedestalfans] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Pedestalfans] Raw amps:', amps);
      console.log('[Pedestalfans] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Pedestalfans'
    };

    if (shouldLog) console.log('[Pedestalfans] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Pedestalfans] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createPedestalfans:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Pedestalfans' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Pedestalfans' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Pedestalfans] Update id:', req.params.id);
      console.log('[Pedestalfans] Body:', req.body);
      console.log('[Pedestalfans] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Pedestalfans' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updatePedestalfans:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Pedestalfans' });
    if (!product) return res.status(404).json({ error: 'Not found' });
