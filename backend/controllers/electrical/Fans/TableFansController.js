...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TableFans] Create request');
      console.log('[TableFans] Body:', req.body);
      console.log('[TableFans] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[TableFans] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[TableFans] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[TableFans] Raw amps:', amps);
      console.log('[TableFans] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'TableFans'
    };

    if (shouldLog) console.log('[TableFans] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[TableFans] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createTableFans:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Tablefans' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Tablefans' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TableFans] Update id:', req.params.id);
      console.log('[TableFans] Body:', req.body);
      console.log('[TableFans] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Tablefans' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateTableFans:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Tablefans' });
    if (!product) return res.status(404).json({ error: 'Not found' });
