...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[WallMountingfans] Create request');
      console.log('[WallMountingfans] Body:', req.body);
      console.log('[WallMountingfans] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[WallMountingfans] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[WallMountingfans] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[WallMountingfans] Raw amps:', amps);
      console.log('[WallMountingfans] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'WallMountingfans'
    };

    if (shouldLog) console.log('[WallMountingfans] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[WallMountingfans] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createWallMountingfans:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'wallmountingfans' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'wallmountingfans' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[WallMountingfans] Update id:', req.params.id);
      console.log('[WallMountingfans] Body:', req.body);
      console.log('[WallMountingfans] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'wallmountingfans' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateWallMountingfans:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'wallmountingfans' });
    if (!product) return res.status(404).json({ error: 'Not found' });
