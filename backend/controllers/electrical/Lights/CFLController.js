...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CFL] Create request');
      console.log('[CFL] Body:', req.body);
      console.log('[CFL] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[CFL] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[CFL] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[CFL] Raw amps:', amps);
      console.log('[CFL] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'CFL'
    };

    if (shouldLog) console.log('[CFL] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[CFL] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createCFL:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Cfl' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Cfl' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[CFL] Update id:', req.params.id);
      console.log('[CFL] Body:', req.body);
      console.log('[CFL] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Cfl' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateCFL:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Cfl' });
    if (!product) return res.status(404).json({ error: 'Not found' });
