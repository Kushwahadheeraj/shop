...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Ceilinglight] Create request');
      console.log('[Ceilinglight] Body:', req.body);
      console.log('[Ceilinglight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Ceilinglight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Ceilinglight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Ceilinglight] Raw amps:', amps);
      console.log('[Ceilinglight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Ceilinglight'
    };

    if (shouldLog) console.log('[Ceilinglight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Ceilinglight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createCeilinglight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Ceilinglight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Ceilinglight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Ceilinglight] Update id:', req.params.id);
      console.log('[Ceilinglight] Body:', req.body);
      console.log('[Ceilinglight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Ceilinglight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateCeilinglight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Ceilinglight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
