...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[FocusLight] Create request');
      console.log('[FocusLight] Body:', req.body);
      console.log('[FocusLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[FocusLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[FocusLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[FocusLight] Raw amps:', amps);
      console.log('[FocusLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'FocusLight'
    };

    if (shouldLog) console.log('[FocusLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[FocusLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createFocusLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'focuslight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'focuslight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[FocusLight] Update id:', req.params.id);
      console.log('[FocusLight] Body:', req.body);
      console.log('[FocusLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'focuslight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateFocusLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'focuslight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
