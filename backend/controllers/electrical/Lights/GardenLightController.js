...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[GardenLight] Create request');
      console.log('[GardenLight] Body:', req.body);
      console.log('[GardenLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[GardenLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[GardenLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[GardenLight] Raw amps:', amps);
      console.log('[GardenLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'GardenLight'
    };

    if (shouldLog) console.log('[GardenLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[GardenLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createGardenLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'gardenlight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'gardenlight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[GardenLight] Update id:', req.params.id);
      console.log('[GardenLight] Body:', req.body);
      console.log('[GardenLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'gardenlight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateGardenLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'gardenlight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
