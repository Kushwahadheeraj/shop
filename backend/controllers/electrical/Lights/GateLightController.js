...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[GateLight] Create request');
      console.log('[GateLight] Body:', req.body);
      console.log('[GateLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[GateLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[GateLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[GateLight] Raw amps:', amps);
      console.log('[GateLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'GateLight'
    };

    if (shouldLog) console.log('[GateLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[GateLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createGateLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'gatelight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'gatelight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[GateLight] Update id:', req.params.id);
      console.log('[GateLight] Body:', req.body);
      console.log('[GateLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'gatelight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateGateLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'gatelight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
