...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Home] Create request');
      console.log('[Home] Body:', req.body);
      console.log('[Home] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Home] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Home] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Home] Raw amps:', amps);
      console.log('[Home] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Home'
    };

    if (shouldLog) console.log('[Home] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Home] Created:', product._id);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Home' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Home' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Home] Update id:', req.params.id);
      console.log('[Home] Body:', req.body);
      console.log('[Home] Files:', req.files);
    }
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Home' });
    if (!product) return res.status(404).json({ error: 'Not found' });
