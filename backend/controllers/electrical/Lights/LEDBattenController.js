...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDBatten] Create request');
      console.log('[LEDBatten] Body:', req.body);
      console.log('[LEDBatten] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LEDBatten] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LEDBatten] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LEDBatten] Raw amps:', amps);
      console.log('[LEDBatten] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LEDBatten'
    };

    if (shouldLog) console.log('[LEDBatten] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LEDBatten] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLEDBatten:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'ledbatten' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'ledbatten' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDBatten] Update id:', req.params.id);
      console.log('[LEDBatten] Body:', req.body);
      console.log('[LEDBatten] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'ledbatten' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLEDBatten:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'ledbatten' });
    if (!product) return res.status(404).json({ error: 'Not found' });
