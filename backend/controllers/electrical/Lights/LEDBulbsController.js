...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDBulbs] Create request');
      console.log('[LEDBulbs] Body:', req.body);
      console.log('[LEDBulbs] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LEDBulbs] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LEDBulbs] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LEDBulbs] Raw amps:', amps);
      console.log('[LEDBulbs] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LEDBulbs'
    };

    if (shouldLog) console.log('[LEDBulbs] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LEDBulbs] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLEDBulbs:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'ledbulbs' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'ledbulbs' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDBulbs] Update id:', req.params.id);
      console.log('[LEDBulbs] Body:', req.body);
      console.log('[LEDBulbs] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'ledbulbs' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLEDBulbs:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'ledbulbs' });
    if (!product) return res.status(404).json({ error: 'Not found' });
