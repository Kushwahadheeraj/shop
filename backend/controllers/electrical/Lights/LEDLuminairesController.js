...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDLuminaires] Create request');
      console.log('[LEDLuminaires] Body:', req.body);
      console.log('[LEDLuminaires] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LEDLuminaires] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LEDLuminaires] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LEDLuminaires] Raw amps:', amps);
      console.log('[LEDLuminaires] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LEDLuminaires'
    };

    if (shouldLog) console.log('[LEDLuminaires] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LEDLuminaires] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLEDLuminaires:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'ledluminaires' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'ledluminaires' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDLuminaires] Update id:', req.params.id);
      console.log('[LEDLuminaires] Body:', req.body);
      console.log('[LEDLuminaires] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'ledluminaires' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLEDLuminaires:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'ledluminaires' });
    if (!product) return res.status(404).json({ error: 'Not found' });
