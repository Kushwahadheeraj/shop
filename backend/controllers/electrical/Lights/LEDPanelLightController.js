...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDPanelLight] Create request');
      console.log('[LEDPanelLight] Body:', req.body);
      console.log('[LEDPanelLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LEDPanelLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LEDPanelLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LEDPanelLight] Raw amps:', amps);
      console.log('[LEDPanelLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LEDPanelLight'
    };

    if (shouldLog) console.log('[LEDPanelLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LEDPanelLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLEDPanelLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'ledpanellight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'ledpanellight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDPanelLight] Update id:', req.params.id);
      console.log('[LEDPanelLight] Body:', req.body);
      console.log('[LEDPanelLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'ledpanellight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLEDPanelLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'ledpanellight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
