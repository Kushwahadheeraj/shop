...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDSpotlight] Create request');
      console.log('[LEDSpotlight] Body:', req.body);
      console.log('[LEDSpotlight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LEDSpotlight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LEDSpotlight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LEDSpotlight] Raw amps:', amps);
      console.log('[LEDSpotlight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LEDSpotlight'
    };

    if (shouldLog) console.log('[LEDSpotlight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LEDSpotlight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLEDSpotlight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'ledspotlight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'ledspotlight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDSpotlight] Update id:', req.params.id);
      console.log('[LEDSpotlight] Body:', req.body);
      console.log('[LEDSpotlight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'ledspotlight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLEDSpotlight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'ledspotlight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
