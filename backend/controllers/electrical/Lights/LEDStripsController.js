...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDStrips] Create request');
      console.log('[LEDStrips] Body:', req.body);
      console.log('[LEDStrips] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LEDStrips] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LEDStrips] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LEDStrips] Raw amps:', amps);
      console.log('[LEDStrips] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LEDStrips'
    };

    if (shouldLog) console.log('[LEDStrips] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LEDStrips] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLEDStrips:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Ledstrips' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Ledstrips' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LEDStrips] Update id:', req.params.id);
      console.log('[LEDStrips] Body:', req.body);
      console.log('[LEDStrips] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Ledstrips' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLEDStrips:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Ledstrips' });
    if (!product) return res.status(404).json({ error: 'Not found' });
