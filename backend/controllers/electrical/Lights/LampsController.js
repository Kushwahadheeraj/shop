...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Lamps] Create request');
      console.log('[Lamps] Body:', req.body);
      console.log('[Lamps] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Lamps] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Lamps] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Lamps] Raw amps:', amps);
      console.log('[Lamps] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Lamps'
    };

    if (shouldLog) console.log('[Lamps] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Lamps] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLamps:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'lamps' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'lamps' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Lamps] Update id:', req.params.id);
      console.log('[Lamps] Body:', req.body);
      console.log('[Lamps] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'lamps' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLamps:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'lamps' });
    if (!product) return res.status(404).json({ error: 'Not found' });
