...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LedSurfaceLight] Create request');
      console.log('[LedSurfaceLight] Body:', req.body);
      console.log('[LedSurfaceLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LedSurfaceLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LedSurfaceLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LedSurfaceLight] Raw amps:', amps);
      console.log('[LedSurfaceLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LedSurfaceLight'
    };

    if (shouldLog) console.log('[LedSurfaceLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LedSurfaceLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLedSurfaceLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Ledsurfacelight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Ledsurfacelight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LedSurfaceLight] Update id:', req.params.id);
      console.log('[LedSurfaceLight] Body:', req.body);
      console.log('[LedSurfaceLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Ledsurfacelight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLedSurfaceLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Ledsurfacelight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
