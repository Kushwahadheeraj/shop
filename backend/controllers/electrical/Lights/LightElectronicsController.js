...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LightElectronics] Create request');
      console.log('[LightElectronics] Body:', req.body);
      console.log('[LightElectronics] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[LightElectronics] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[LightElectronics] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[LightElectronics] Raw amps:', amps);
      console.log('[LightElectronics] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'LightElectronics'
    };

    if (shouldLog) console.log('[LightElectronics] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[LightElectronics] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createLightElectronics:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'lightelectronics' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'lightelectronics' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[LightElectronics] Update id:', req.params.id);
      console.log('[LightElectronics] Body:', req.body);
      console.log('[LightElectronics] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'lightelectronics' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateLightElectronics:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'lightelectronics' });
    if (!product) return res.status(404).json({ error: 'Not found' });
