...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[MirrorLight] Create request');
      console.log('[MirrorLight] Body:', req.body);
      console.log('[MirrorLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[MirrorLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[MirrorLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[MirrorLight] Raw amps:', amps);
      console.log('[MirrorLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'MirrorLight'
    };

    if (shouldLog) console.log('[MirrorLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[MirrorLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createMirrorLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Mirrorlight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Mirrorlight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[MirrorLight] Update id:', req.params.id);
      console.log('[MirrorLight] Body:', req.body);
      console.log('[MirrorLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Mirrorlight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateMirrorLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Mirrorlight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
