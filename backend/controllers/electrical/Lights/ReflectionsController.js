...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Reflections] Create request');
      console.log('[Reflections] Body:', req.body);
      console.log('[Reflections] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Reflections] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Reflections] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Reflections] Raw amps:', amps);
      console.log('[Reflections] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Reflections'
    };

    if (shouldLog) console.log('[Reflections] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Reflections] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createReflections:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Reflections' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Reflections' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Reflections] Update id:', req.params.id);
      console.log('[Reflections] Body:', req.body);
      console.log('[Reflections] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Reflections' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateReflections:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Reflections' });
    if (!product) return res.status(404).json({ error: 'Not found' });
