...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TBulb] Create request');
      console.log('[TBulb] Body:', req.body);
      console.log('[TBulb] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[TBulb] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[TBulb] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[TBulb] Raw amps:', amps);
      console.log('[TBulb] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'TBulb'
    };

    if (shouldLog) console.log('[TBulb] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[TBulb] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createTBulb:', err);
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TBulb] Update id:', req.params.id);
      console.log('[TBulb] Body:', req.body);
      console.log('[TBulb] Files:', req.files);
    }
    ...
    console.error('Error in updateTBulb:', err);
    res.status(500).json({ error: err.message });
  }
};
