...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TubeLight] Create request');
      console.log('[TubeLight] Body:', req.body);
      console.log('[TubeLight] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[TubeLight] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[TubeLight] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[TubeLight] Raw amps:', amps);
      console.log('[TubeLight] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'TubeLight'
    };

    if (shouldLog) console.log('[TubeLight] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[TubeLight] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createTubeLight:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Tubelight' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Tubelight' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TubeLight] Update id:', req.params.id);
      console.log('[TubeLight] Body:', req.body);
      console.log('[TubeLight] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Tubelight' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateTubeLight:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Tubelight' });
    if (!product) return res.status(404).json({ error: 'Not found' });
