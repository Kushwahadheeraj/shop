...
exports.create = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[UnderWaterLights] Create request');
      console.log('[UnderWaterLights] Body:', req.body);
      console.log('[UnderWaterLights] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[UnderWaterLights] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[UnderWaterLights] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[UnderWaterLights] Raw amps:', amps);
      console.log('[UnderWaterLights] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'UnderWaterLights'
    };

    if (shouldLog) console.log('[UnderWaterLights] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[UnderWaterLights] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createUnderWaterLights:', err);
...
exports.getAll = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'underwaterlights' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOne = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'underwaterlights' });
...
exports.update = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[UnderWaterLights] Update id:', req.params.id);
      console.log('[UnderWaterLights] Body:', req.body);
      console.log('[UnderWaterLights] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'underwaterlights' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateUnderWaterLights:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.delete = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'underwaterlights' });
    if (!product) return res.status(404).json({ error: 'Not found' });
