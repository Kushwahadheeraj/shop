...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'image', folder: 'wiresAndCables' },
      (error, result) => {
...
exports.createWiresAndCables = async (req, res) => {
  try {
    ...
    const wiresAndCables = new Electrical({ ...req.body, photos: photoUrls, type: 'WiresAndCables' });
    await wiresAndCables.save();
    res.status(201).json(wiresAndCables);
  } catch (err) {
...
// Get Fan by ID
exports.getWiresAndCablesById = async (req, res) => {
  try {
