...
exports.createDimmer = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Dimmer] Create request');
      console.log('[Dimmer] Body:', req.body);
      console.log('[Dimmer] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Dimmer] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Dimmer] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Dimmer] Raw amps:', amps);
      console.log('[Dimmer] Raw tag:', tag);
    }
    ...
    if (shouldLog) console.log('[Dimmer] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Dimmer] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createDimmer:', err);
...
exports.updateDimmer = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Dimmer] Update id:', req.params.id);
      console.log('[Dimmer] Body:', req.body);
      console.log('[Dimmer] Files:', req.files);
    }
    ...
    console.error('Error in updateDimmer:', err);
    res.status(500).json({ error: err.message });
  }
};
