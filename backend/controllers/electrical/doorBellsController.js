...
/**
 * Create a new DoorBells product.
 */
exports.createDoorBells = async (req, res) => {
  try {
    ...
    const product = new DoorBellsModel({ ...req.body, photos: photoUrls, category: 'doorBells' });
    await product.save();
...
/**
 * Update a DoorBells product by ID.
 */
exports.updateDoorBells = async (req, res) => {
  try {
    ...
    const product = await DoorBellsModel.findOneAndUpdate(
