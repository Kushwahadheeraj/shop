...
exports.createELCBsRCCBs = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[ELCBsRCCBs] Create request');
      console.log('[ELCBsRCCBs] Body:', req.body);
      console.log('[ELCBsRCCBs] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[ELCBsRCCBs] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[ELCBsRCCBs] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[ELCBsRCCBs] Raw amps:', amps);
      console.log('[ELCBsRCCBs] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'ELCBsRCCBs'
    };

    if (shouldLog) console.log('[ELCBsRCCBs] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[ELCBsRCCBs] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createELCBsRCCBs:', err);
...
exports.getAllELCBsRCCBs = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Elcbsrccbs' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneELCBsRCCBs = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Elcbsrccbs' });
...
exports.updateELCBsRCCBs = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[ELCBsRCCBs] Update id:', req.params.id);
      console.log('[ELCBsRCCBs] Body:', req.body);
      console.log('[ELCBsRCCBs] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Elcbsrccbs' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateELCBsRCCBs:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deleteELCBsRCCBs = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Elcbsrccbs' });
    if (!product) return res.status(404).json({ error: 'Not found' });
