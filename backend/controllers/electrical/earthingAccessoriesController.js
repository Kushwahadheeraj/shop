...
exports.createEarthingAccessories = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[EarthingAccessories] Create request');
      console.log('[EarthingAccessories] Body:', req.body);
      console.log('[EarthingAccessories] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[EarthingAccessories] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[EarthingAccessories] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[EarthingAccessories] Raw amps:', amps);
      console.log('[EarthingAccessories] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'EarthingAccessories'
    };

    if (shouldLog) console.log('[EarthingAccessories] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[EarthingAccessories] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createEarthingAccessories:', err);
...
exports.updateEarthingAccessories = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[EarthingAccessories] Update id:', req.params.id);
      console.log('[EarthingAccessories] Body:', req.body);
      console.log('[EarthingAccessories] Files:', req.files);
    }
    ...
    console.error('Error in updateEarthingAccessories:', err);
    res.status(500).json({ error: err.message });
  }
};
