...
exports.createFan = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Fan] Create request');
      console.log('[Fan] Body:', req.body);
      console.log('[Fan] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Fan] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Fan] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Fan] Raw amps:', amps);
      console.log('[Fan] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Fan'
    };

    if (shouldLog) console.log('[Fan] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Fan] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createFan:', err);
...
exports.updateFan = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Fan] Update id:', req.params.id);
      console.log('[Fan] Body:', req.body);
      console.log('[Fan] Files:', req.files);
    }
    ...
    console.error('Error in updateFan:', err);
    res.status(500).json({ error: err.message });
  }
};
