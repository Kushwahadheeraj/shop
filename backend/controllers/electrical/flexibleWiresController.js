...
exports.createFlexibleWires = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[FlexibleWires] Create request');
      console.log('[FlexibleWires] Body:', req.body);
      console.log('[FlexibleWires] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[FlexibleWires] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[FlexibleWires] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[FlexibleWires] Raw amps:', amps);
      console.log('[FlexibleWires] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'FlexibleWires'
    };

    if (shouldLog) console.log('[FlexibleWires] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[FlexibleWires] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createFlexibleWires:', err);
...
exports.updateFlexibleWires = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[FlexibleWires] Update id:', req.params.id);
      console.log('[FlexibleWires] Body:', req.body);
      console.log('[FlexibleWires] Files:', req.files);
    }
    ...
    console.error('Error in updateFlexibleWires:', err);
    res.status(500).json({ error: err.message });
  }
};
