...
exports.createHolders = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Holders] Create request');
      console.log('[Holders] Body:', req.body);
      console.log('[Holders] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Holders] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Holders] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Holders] Raw amps:', amps);
      console.log('[Holders] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Holders'
    };

    if (shouldLog) console.log('[Holders] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Holders] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createHolders:', err);
...
exports.getAllHolders = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Holders' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHolders = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Holders' });
...
exports.updateHolders = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Holders] Update id:', req.params.id);
      console.log('[Holders] Body:', req.body);
      console.log('[Holders] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Holders' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateHolders:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deleteHolders = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Holders' });
    if (!product) return res.status(404).json({ error: 'Not found' });
