...
exports.createIndicator = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Indicator] Create request');
      console.log('[Indicator] Body:', req.body);
      console.log('[Indicator] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Indicator] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Indicator] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Indicator] Raw amps:', amps);
      console.log('[Indicator] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Indicator'
    };

    if (shouldLog) console.log('[Indicator] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Indicator] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createIndicator:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.getAllIndicator = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'indicator' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneIndicator = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'indicator' });
...
exports.updateIndicator = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Indicator] Update id:', req.params.id);
      console.log('[Indicator] Body:', req.body);
      console.log('[Indicator] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'indicator' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateIndicator:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.deleteIndicator = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'indicator' });
    if (!product) return res.status(404).json({ error: 'Not found' });
