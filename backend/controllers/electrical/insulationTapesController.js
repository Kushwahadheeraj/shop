...
exports.createInsulationTapes = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[InsulationTapes] Create request');
      console.log('[InsulationTapes] Body:', req.body);
      console.log('[InsulationTapes] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[InsulationTapes] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[InsulationTapes] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[InsulationTapes] Raw amps:', amps);
      console.log('[InsulationTapes] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'InsulationTapes'
    };

    if (shouldLog) console.log('[InsulationTapes] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[InsulationTapes] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createInsulationTapes:', err);
...
exports.getAllInsulationTapes = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Insulationtapes' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.updateInsulationTapes = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[InsulationTapes] Update id:', req.params.id);
      console.log('[InsulationTapes] Body:', req.body);
      console.log('[InsulationTapes] Files:', req.files);
    }
    ...
    console.error('Error in updateInsulationTapes:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deleteInsulationTapes = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Insulationtapes' });
    if (!product) return res.status(404).json({ error: 'Not found' });
