...
exports.createIsolators = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Isolators] Create request');
      console.log('[Isolators] Body:', req.body);
      console.log('[Isolators] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Isolators] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Isolators] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Isolators] Raw amps:', amps);
      console.log('[Isolators] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Isolators'
    };

    if (shouldLog) console.log('[Isolators] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Isolators] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createIsolators:', err);
...
exports.getAllIsolators = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'isolators' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneIsolators = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'isolators' });
...
exports.updateIsolators = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Isolators] Update id:', req.params.id);
      console.log('[Isolators] Body:', req.body);
      console.log('[Isolators] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'isolators' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateIsolators:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deleteIsolators = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'isolators' });
    if (!product) return res.status(404).json({ error: 'Not found' });
