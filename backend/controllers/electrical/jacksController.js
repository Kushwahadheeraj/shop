...
exports.createJacks = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Jacks] Create request');
      console.log('[Jacks] Body:', req.body);
      console.log('[Jacks] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Jacks] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Jacks] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Jacks] Raw amps:', amps);
      console.log('[Jacks] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Jacks'
    };

    if (shouldLog) console.log('[Jacks] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Jacks] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createJacks:', err);
...
exports.updateJacks = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Jacks] Update id:', req.params.id);
      console.log('[Jacks] Body:', req.body);
      console.log('[Jacks] Files:', req.files);
    }
    ...
    console.error('Error in updateJacks:', err);
    res.status(500).json({ error: err.message });
  }
};
