...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'image', folder: 'mainSwitch' },
      (error, result) => {
...
exports.createMainSwitch = async (req, res) => {
  try {
    ...
    const mainSwitch = new Electrical({ ...req.body, photos: photoUrls, type: 'MainSwitch' });
    await mainSwitch.save();
    res.status(201).json(mainSwitch);
  } catch (err) {
...
// Get All Fans
exports.getAllMainSwitchs = async (req, res) => {
  try {
...
// Get Fan by ID
exports.getMainSwitchById = async (req, res) => {
  try {
