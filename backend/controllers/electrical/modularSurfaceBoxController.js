...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'image', folder: 'modularSurfaceBox' },
      (error, result) => {
...
exports.createModularSurfaceBox = async (req, res) => {
  try {
    ...
    const modularSurfaceBox = new Electrical({ ...req.body, photos: photoUrls, type: 'ModularSurfaceBox' });
    await modularSurfaceBox.save();
    res.status(201).json(modularSurfaceBox);
  } catch (err) {
