...
exports.createModularSurfaceBox = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[ModularSurfaceBox] Create request');
      console.log('[ModularSurfaceBox] Body:', req.body);
      console.log('[ModularSurfaceBox] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[ModularSurfaceBox] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[ModularSurfaceBox] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[ModularSurfaceBox] Raw amps:', amps);
      console.log('[ModularSurfaceBox] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'ModularSurfaceBox'
    };

    if (shouldLog) console.log('[ModularSurfaceBox] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[ModularSurfaceBox] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createModularSurfaceBox:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.getAllModularSurfaceBox = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'modularsurfacebox' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneModularSurfaceBox = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'modularsurfacebox' });
...
exports.updateModularSurfaceBox = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[ModularSurfaceBox] Update id:', req.params.id);
      console.log('[ModularSurfaceBox] Body:', req.body);
      console.log('[ModularSurfaceBox] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'modularsurfacebox' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateModularSurfaceBox:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.deleteModularSurfaceBox = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'modularsurfacebox' });
    if (!product) return res.status(404).json({ error: 'Not found' });
