...
exports.createMotors = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Motors] Create request');
      console.log('[Motors] Body:', req.body);
      console.log('[Motors] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Motors] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Motors] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Motors] Raw amps:', amps);
      console.log('[Motors] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Motors'
    };

    if (shouldLog) console.log('[Motors] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Motors] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createMotors:', err);
...
exports.getAllMotors = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Motors' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMotors = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Motors' });
...
exports.updateMotors = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Motors] Update id:', req.params.id);
      console.log('[Motors] Body:', req.body);
      console.log('[Motors] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Motors' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateMotors:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deleteMotors = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Motors' });
    if (!product) return res.status(404).json({ error: 'Not found' });
