...
exports.createOthers = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Others] Create request');
      console.log('[Others] Body:', req.body);
      console.log('[Others] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Others] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Others] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Others] Raw amps:', amps);
      console.log('[Others] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Others'
    };

    if (shouldLog) console.log('[Others] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Others] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createOthers:', err);
...
exports.getAllOthers = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'others' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneOthers = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'others' });
...
exports.updateOthers = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Others] Update id:', req.params.id);
      console.log('[Others] Body:', req.body);
      console.log('[Others] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'others' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateOthers:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.deleteOthers = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'others' });
    if (!product) return res.status(404).json({ error: 'Not found' });
