...
exports.createPinTop = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[PinTop] Create request');
      console.log('[PinTop] Body:', req.body);
      console.log('[PinTop] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[PinTop] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[PinTop] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[PinTop] Raw amps:', amps);
      console.log('[PinTop] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'PinTop'
    };

    if (shouldLog) console.log('[PinTop] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[PinTop] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createPinTop:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.getAllPinTop = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'Pintop' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePinTop = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'Pintop' });
...
exports.updatePinTop = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[PinTop] Update id:', req.params.id);
      console.log('[PinTop] Body:', req.body);
      console.log('[PinTop] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'Pintop' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updatePinTop:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deletePinTop = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'Pintop' });
    if (!product) return res.status(404).json({ error: 'Not found' });
