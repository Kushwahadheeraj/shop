...
exports.createPlug = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Plug] Create request');
      console.log('[Plug] Body:', req.body);
      console.log('[Plug] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[Plug] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[Plug] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[Plug] Raw amps:', amps);
      console.log('[Plug] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'Plug'
    };

    if (shouldLog) console.log('[Plug] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[Plug] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createPlug:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.getAllPlug = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'plug' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePlug = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'plug' });
...
exports.updatePlug = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[Plug] Update id:', req.params.id);
      console.log('[Plug] Body:', req.body);
      console.log('[Plug] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'plug' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updatePlug:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.deletePlug = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'plug' });
    if (!product) return res.status(404).json({ error: 'Not found' });
