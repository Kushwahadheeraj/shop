...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'image', folder: 'switchPlates' },
      (error, result) => {
...
exports.createSwitchPlate = async (req, res) => {
  try {
    ...
    const switchPlate = new Electrical({ ...req.body, photos: photoUrls, type: 'SwitchPlate' });
    await switchPlate.save();
    res.status(201).json(switchPlate);
  } catch (err) {
