...
exports.createTVOutlets = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TVOutlets] Create request');
      console.log('[TVOutlets] Body:', req.body);
      console.log('[TVOutlets] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[TVOutlets] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[TVOutlets] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[TVOutlets] Raw amps:', amps);
      console.log('[TVOutlets] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'TVOutlets'
    };

    if (shouldLog) console.log('[TVOutlets] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[TVOutlets] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createTVOutlets:', err);
...
exports.getAllTVOutlets = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'tvoutlets' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTVOutlets = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'tvoutlets' });
...
exports.updateTVOutlets = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[TVOutlets] Update id:', req.params.id);
      console.log('[TVOutlets] Body:', req.body);
      console.log('[TVOutlets] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'tvoutlets' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateTVOutlets:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.deleteTVOutlets = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'tvoutlets' });
    if (!product) return res.status(404).json({ error: 'Not found' });
