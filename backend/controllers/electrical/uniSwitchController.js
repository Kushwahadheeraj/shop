...
exports.createUniSwitch = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[UniSwitch] Create request');
      console.log('[UniSwitch] Body:', req.body);
      console.log('[UniSwitch] Files:', req.files);
    }
    ...
    if (shouldLog) console.log('[UniSwitch] Uploading images to Cloudinary...');
    const photoUrls = await Promise.all(req.files.map(file => uploadToCloudinary(file.buffer)));
    if (shouldLog) console.log('[UniSwitch] Uploaded URLs:', photoUrls);

    // Parse amps and tag if sent as JSON string or array entries
    let { amps, tag, ...rest } = req.body;
    if (shouldLog) {
      console.log('[UniSwitch] Raw amps:', amps);
      console.log('[UniSwitch] Raw tag:', tag);
    }
    ...
    const productData = {
      ...
      tag,
      photos: photoUrls,
      category: rest.category || 'UniSwitch'
    };

    if (shouldLog) console.log('[UniSwitch] Creating with data:', productData);
    const product = new ElectricalModels(productData);
    await product.save();
    if (shouldLog) console.log('[UniSwitch] Created:', product._id);

    res.status(201).json(product);
  } catch (err) {
    console.error('Error in createUniSwitch:', err);
    res.status(500).json({ error: err.message });
  }
};

exports.getAllUniSwitch = async (req, res) => {
  try {
    const products = await ElectricalModels.find({ category: 'uniswitch' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneUniSwitch = async (req, res) => {
  try {
    const product = await ElectricalModels.findOne({ _id: req.params.id, category: 'uniswitch' });
...
exports.updateUniSwitch = async (req, res) => {
  try {
    if (shouldLog) {
      console.log('[UniSwitch] Update id:', req.params.id);
      console.log('[UniSwitch] Body:', req.body);
      console.log('[UniSwitch] Files:', req.files);
    }
    ...
    const product = await ElectricalModels.findOneAndUpdate(
      { _id: req.params.id, category: 'uniswitch' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
    ...
    console.error('Error in updateUniSwitch:', err);
    res.status(500).json({ error: err.message });
  }
};
...
exports.deleteUniSwitch = async (req, res) => {
  try {
    const product = await ElectricalModels.findOneAndDelete({ _id: req.params.id, category: 'uniswitch' });
    if (!product) return res.status(404).json({ error: 'Not found' });
