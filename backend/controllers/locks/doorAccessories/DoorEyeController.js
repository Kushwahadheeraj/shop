...
exports.createDoorEye = async (req, res) => {
  try {
    ...
    const product = new DoorEyeModel({ ...req.body, photos: photoUrls, category: 'DoorEye' });
    await product.save();
...
/**
 * Update a DoorEye product by ID.
 */
exports.updateDoorEye = async (req, res) => {
  try {
    ...
    const product = await DoorEyeModel.findOneAndUpdate(
      { _id: req.params.id, category: 'DoorEye' },
      update,
      { new: true }
    );
