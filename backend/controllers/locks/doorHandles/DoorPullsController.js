...
exports.createDoorPulls = async (req, res) => {
  try {
    ...
    const product = new DoorPullsModel({ ...req.body, photos: photoUrls, category: 'DoorPulls' });
    await product.save();
...
/**
 * Update a DoorPulls product by ID.
 */
exports.updateDoorPulls = async (req, res) => {
  try {
    ...
    const product = await DoorPullsModel.findOneAndUpdate(
      { _id: req.params.id, category: 'DoorPulls' },
      update,
      { new: true }
    );
