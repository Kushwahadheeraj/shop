...
exports.getAllCentreShutterLocks = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'CentreShutterLocks' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deleteCentreShutterLocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'CentreShutterLocks' });
...
exports.getOneCentreShutterLocks = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'CentreShutterLocks' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
