...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'CentreShutterLocks', folder: 'centreshutterlocks' },
      (error, result) => {
...
exports.createCentreShutterLocks = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'CentreShutterLocks' });
    await item.save();
...
exports.getCentreShutterLocksById = async (req, res) => {
  try {
...
exports.updateCentreShutterLocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'CentreShutterLocks' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
