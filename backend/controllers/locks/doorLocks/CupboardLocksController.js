...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'CupboardLocks', folder: 'cupboardlocks' },
      (error, result) => {
...
exports.createCupboardLocks = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'CupboardLocks' });
    await item.save();
...
exports.getCupboardLocksById = async (req, res) => {
  try {
...
exports.updateCupboardLocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'CupboardLocks' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
