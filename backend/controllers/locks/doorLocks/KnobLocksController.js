...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'KnobLocks', folder: 'knoblocks' },
      (error, result) => {
...
exports.createKnobLocks = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'KnobLocks' });
    await item.save();
...
exports.getKnobLocksById = async (req, res) => {
  try {
...
exports.updateKnobLocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'KnobLocks' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
