...
/**
 * Create a new PullHandlesForMainDoors product.
 */
exports.createPullHandlesForMainDoors = async (req, res) => {
  try {
    ...
    const product = new PullHandlesForMainDoorsModel({ ...req.body, photos: photoUrls, category: 'PullHandlesForMainDoors' });
    await product.save();
...
/**
 * Update a PullHandlesForMainDoors product by ID.
 */
exports.updatePullHandlesForMainDoors = async (req, res) => {
  try {
    ...
    const product = await PullHandlesForMainDoorsModel.findOneAndUpdate(
      { _id: req.params.id, category: 'PullHandlesForMainDoors' },
      update,
      { new: true }
    );
