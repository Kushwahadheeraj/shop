...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'SmartKey', folder: 'smartkey' },
      (error, result) => {
...
exports.createSmartKey = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'SmartKey' });
    await item.save();
...
exports.getSmartKeyById = async (req, res) => {
  try {
...
exports.updateSmartKey = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'SmartKey' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
