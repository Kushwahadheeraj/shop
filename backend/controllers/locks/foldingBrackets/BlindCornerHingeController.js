...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'BlindCornerHinge', folder: 'blindcornerhinge' },
      (error, result) => {
...
exports.createBlindCornerHinge = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'BlindCornerHinge' });
    await item.save();
...
exports.getBlindCornerHingeById = async (req, res) => {
  try {
...
exports.updateBlindCornerHinge = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'BlindCornerHinge' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
