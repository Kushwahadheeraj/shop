...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'ClipOnSoftHinge', folder: 'cliponsofthinge' },
      (error, result) => {
...
exports.createClipOnSoftHinge = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'ClipOnSoftHinge' });
    await item.save();
...
exports.getClipOnSoftHingeById = async (req, res) => {
  try {
...
exports.updateClipOnSoftHinge = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'ClipOnSoftHinge' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
