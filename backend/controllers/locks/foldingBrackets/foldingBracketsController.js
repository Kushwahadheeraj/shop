...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'foldingBrackets', folder: 'foldingbrackets' },
      (error, result) => {
...
exports.createfoldingBrackets = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'foldingBrackets' });
    await item.save();
...
exports.getfoldingBracketsById = async (req, res) => {
  try {
...
exports.updatefoldingBrackets = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'foldingBrackets' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
