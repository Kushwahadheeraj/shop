...
/**
 * Create a new TableLock product.
 */
exports.createTableLock = async (req, res) => {
  try {
    ...
    const product = new TableLockModel({ ...req.body, photos: photoUrls, category: 'TableLock' });
    await product.save();
...
/**
 * Update a TableLock product by ID.
 */
exports.updateTableLock = async (req, res) => {
  try {
    ...
    const product = await TableLockModel.findOneAndUpdate(
      { _id: req.params.id, category: 'TableLock' },
      update,
      { new: true }
    );
