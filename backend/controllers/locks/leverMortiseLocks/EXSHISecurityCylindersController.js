...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'EXSHISecurityCylinders', folder: 'exshisecuritycylinders' },
      (error, result) => {
...
exports.createEXSHISecurityCylinders = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'EXSHISecurityCylinders' });
    await item.save();
...
exports.getEXSHISecurityCylindersById = async (req, res) => {
  try {
...
exports.updateEXSHISecurityCylinders = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'EXSHISecurityCylinders' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
