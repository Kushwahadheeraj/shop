...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'leverMortiseLocks', folder: 'levermortiselocks' },
      (error, result) => {
...
exports.createleverMortiseLocks = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'leverMortiseLocks' });
    await item.save();
...
exports.getleverMortiseLocksById = async (req, res) => {
  try {
...
exports.updateleverMortiseLocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'leverMortiseLocks' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
