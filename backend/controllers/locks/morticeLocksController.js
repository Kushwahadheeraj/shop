...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'morticeLocks', folder: 'morticelocks' },
      (error, result) => {
...
exports.createmorticeLocks = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'morticeLocks' });
    await item.save();
...
exports.getmorticeLocksById = async (req, res) => {
  try {
...
exports.updatemorticeLocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'morticeLocks' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
