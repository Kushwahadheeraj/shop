...
/**
 * Create a new MortiseLockBody product.
 */
exports.createMortiseLockBody = async (req, res) => {
  try {
    ...
    const product = new locksModel({ ...req.body, photos: photoUrls, category: 'mortiseLockBody' });
    await product.save();
...
/**
 * Update a MortiseLockBody product by ID.
 */
exports.updateMortiseLockBody = async (req, res) => {
  try {
    ...
    const product = await locksModel.findOneAndUpdate(
      { _id: req.params.id, category: 'mortiseLockBody' },
      update,
      { new: true }
    );
