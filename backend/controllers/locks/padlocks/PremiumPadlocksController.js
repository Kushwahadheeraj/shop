...
exports.createPremiumPadlocks = async (req, res) => {
  try {
    ...
    const product = new Lock({
      ...
      type: 'PremiumPadlocks',
      productNo: req.body.productNo || 'PP-' + Date.now(),
...
exports.getAllPremiumPadlocks = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'PremiumPadlocks' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deletePremiumPadlocks = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'PremiumPadlocks' });
...
exports.getOnePremiumPadlocks = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'PremiumPadlocks' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
