...
exports.updateBM04 = async (req, res) => {
  try {
    ...
    const product = await Lock.findOneAndUpdate(
      { _id: req.params.id, category: 'BM04' },
      update,
      { new: true }
    );
...
exports.getAllBM04 = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'BM04' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deleteBM04 = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'BM04' });
...
exports.getOneBM04 = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'BM04' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
