...
exports.createBM06 = async (req, res) => {
  try {
    ...
    const product = new BM06Model({ ...req.body, photos: photoUrls, category: 'BM06' });
    await product.save();
...
exports.updateBM06 = async (req, res) => {
  try {
    ...
    const product = await BM06Model.findOneAndUpdate(
      { _id: req.params.id, category: 'BM06' },
      update,
      { new: true }
    );
