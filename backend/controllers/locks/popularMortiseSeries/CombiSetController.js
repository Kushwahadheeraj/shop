...
/**
 * Update a CombiSet product by ID.
 */
exports.updateCombiSet = async (req, res) => {
  try {
    ...
    const product = await Lock.findOneAndUpdate(
      { _id: req.params.id, category: 'CombiSet' },
      update,
      { new: true }
    );
...
exports.getAllCombiSet = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'CombiSet' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deleteCombiSet = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'CombiSet' });
...
exports.getOneCombiSet = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'CombiSet' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
