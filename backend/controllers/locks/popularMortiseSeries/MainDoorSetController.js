...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'MainDoorSet', folder: 'maindoorset' },
      (error, result) => {
...
exports.createMainDoorSet = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'MainDoorSet' });
    await item.save();
...
exports.getMainDoorSetById = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, type: 'MainDoorSet' });
...
exports.updateMainDoorSet = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'MainDoorSet' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
