...
exports.createMainDoorSet = async (req, res) => {
  try {
    ...
    const product = new Lock({ ...req.body, photos: photoUrls, category: 'MainDoorSet' });
    await product.save();
...
exports.getAllMainDoorSet = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'MainDoorSet' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deleteMainDoorSet = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'MainDoorSet' });
...
exports.getOneMainDoorSet = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'MainDoorSet' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
