...
exports.createNEH07 = async (req, res) => {
  try {
    ...
    const product = new NEH07Model({ ...req.body, photos: photoUrls, category: 'NEH07' });
    await product.save();
...
exports.updateNEH07 = async (req, res) => {
  try {
    ...
    const product = await NEH07Model.findOneAndUpdate(
      { _id: req.params.id, category: 'NEH07' },
      update,
      { new: true }
    );
