...
/**
 * Create a new NEH15LowHeightDesign product.
 */
exports.createNEH15LowHeightDesign = async (req, res) => {
  try {
    ...
    const product = new NEH15LowHeightDesignModel({ ...req.body, photos: photoUrls, category: 'NEH15LowHeightDesign' });
    await product.save();
...
/**
 * Update a NEH15LowHeightDesign product by ID.
 */
exports.updateNEH15LowHeightDesign = async (req, res) => {
  try {
    ...
    const product = await NEH15LowHeightDesignModel.findOneAndUpdate(
      { _id: req.params.id, category: 'NEH15LowHeightDesign' },
      update,
      { new: true }
    );
