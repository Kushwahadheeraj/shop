...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'PullHandles', folder: 'pullhandles' },
      (error, result) => {
...
exports.createPullHandles = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'PullHandles' });
    await item.save();
...
exports.getPullHandlesById = async (req, res) => {
  try {
...
exports.updatePullHandles = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'PullHandles' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
