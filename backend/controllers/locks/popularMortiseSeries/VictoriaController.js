...
exports.createVictoria = async (req, res) => {
  try {
    ...
    const product = new Lock({
      ...
      category: 'Victoria',
      type: 'Victoria',
      productNo: req.body.productNo || 'VC-' + Date.now(),
...
exports.getAllVictoria = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'Victoria' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deleteVictoria = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'Victoria' });
...
exports.getOneVictoria = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'Victoria' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
