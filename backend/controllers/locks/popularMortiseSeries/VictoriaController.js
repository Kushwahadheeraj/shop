...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'Victoria', folder: 'victoria' },
      (error, result) => {
...
exports.createVictoria = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'Victoria' });
    await item.save();
...
exports.getVictoriaById = async (req, res) => {
  try {
...
exports.updateVictoria = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'Victoria' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
