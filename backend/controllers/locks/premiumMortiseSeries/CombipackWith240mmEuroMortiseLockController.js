...
exports.createCombipackWith240mmEuroMortiseLock = async (req, res) => {
  try {
    ...
    const product = new Lock({ 
      ...
      type: 'CombipackWith240mmEuroMortiseLock',
      productNo: req.body.productNo || 'C240-' + Date.now(),
...
exports.getAllCombipackWith240mmEuroMortiseLock = async (req, res) => {
  try {
    const items = await Lock.find({ category: 'CombipackWith240mmEuroMortiseLock' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.deleteCombipackWith240mmEuroMortiseLock = async (req, res) => {
  try {
    const item = await Lock.findOneAndDelete({ _id: req.params.id, category: 'CombipackWith240mmEuroMortiseLock' });
...
exports.getOneCombipackWith240mmEuroMortiseLock = async (req, res) => {
  try {
    const item = await Lock.findOne({ _id: req.params.id, category: 'CombipackWith240mmEuroMortiseLock' });
    if (!item) return res.status(404).json({ error: 'Not found' });
    res.json(item);
  } catch (err) {
    res.status(500).json({ error: err.message });
