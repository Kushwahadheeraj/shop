...
function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { resource_type: 'CombipackWith240mmEuroMortiseLock', folder: 'combipackwith240mmeuromortiselock' },
      (error, result) => {
...
exports.createCombipackWith240mmEuroMortiseLock = async (req, res) => {
  try {
    ...
    const item = new Lock({ ...req.body, photos: photoUrls, type: 'CombipackWith240mmEuroMortiseLock' });
    await item.save();
...
exports.getCombipackWith240mmEuroMortiseLockById = async (req, res) => {
  try {
...
exports.updateCombipackWith240mmEuroMortiseLock = async (req, res) => {
  try {
    const item = await Lock.findOneAndUpdate(
      { _id: req.params.id, type: 'CombipackWith240mmEuroMortiseLock' },
      req.body,
      { new: true }
    );
    if (!item) return res.status(404).json({ message: 'Not found' });
