...
exports.createPhoenix = async (req, res) => {
  try {
    ...
    const product = new PhoenixModel({ ...req.body, photos: photoUrls, category: 'Phoenix' });
    await product.save();
...
/**
 * Update a Phoenix product by ID.
 */
exports.updatePhoenix = async (req, res) => {
  try {
    ...
    const product = await PhoenixModel.findOneAndUpdate(
      { _id: req.params.id, category: 'Phoenix' },
      update,
      { new: true }
    );
