...
/**
 * Create a new SEHSeries product.
 */
exports.createSEHSeries = async (req, res) => {
  try {
    ...
    const product = new SEHSeriesModel({ ...req.body, photos: photoUrls, category: 'SEHSeries' });
    await product.save();
...
/**
 * Update a SEHSeries product by ID.
 */
exports.updateSEHSeries = async (req, res) => {
  try {
    ...
    const product = await SEHSeriesModel.findOneAndUpdate(
      { _id: req.params.id, category: 'SEHSeries' },
      update,
      { new: true }
    );
