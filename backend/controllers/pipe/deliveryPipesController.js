...
exports.createDeliveryPipe = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'DeliveryPipe' };
...
exports.updateDeliveryPipe = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Pipe.findOneAndUpdate(
      { _id: req.params.id, category: 'DeliveryPipe' },
      update,
      { new: true }
    );
...
exports.getAllDeliveryPipe = async (req, res) => {
  try {
    const items = await Pipe.find({ category: 'DeliveryPipe' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDeliveryPipe = async (req, res) => {
  try {
    const item = await Pipe.findOne({ _id: req.params.id, category: 'DeliveryPipe' });
...
exports.deleteDeliveryPipe = async (req, res) => {
  try {
    const item = await Pipe.findOneAndDelete({ _id: req.params.id, category: 'DeliveryPipe' });
    if (!item) return res.status(404).json({ error: 'Not found' });
