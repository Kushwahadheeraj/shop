 ...
 */
exports.createPrinziaPipes = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PrinziaPipes' };
 ...
 */
exports.updatePrinziaPipes = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    if (update.maxPrice && update.minPrice && update.maxPrice < update.minPrice) {
      return res.status(400).json({ error: 'Max price must be greater than or equal to min price' });
        ...
        { _id: req.params.id, category: 'PrinziaPipes' },
        update,
        { new: true }
      );
...
exports.getAllPrinziaPipes = async (req, res) => {
  try {
    const items = await Pipe.find({ category: 'PrinziaPipes' });
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePrinziaPipes = async (req, res) => {
  try {
    const item = await Pipe.findOne({ _id: req.params.id, category: 'PrinziaPipes' });
...
exports.deletePrinziaPipes = async (req, res) => {
  try {
    const item = await Pipe.findOneAndDelete({ _id: req.params.id, category: 'PrinziaPipes' });
    if (!item) return res.status(404).json({ error: 'Not found' });
