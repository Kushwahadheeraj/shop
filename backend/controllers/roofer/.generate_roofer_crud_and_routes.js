...
exports.create = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/PvcMatsModels'))({ ...req.body, photos: photoUrls, category: '${category}' });
    await product.save();
...
exports.getAll = async (req, res) => {
  try {
    const products = await require('../../models/PvcMatsModels').find({ category: '${category}' });
    res.json(products);
  } catch (err) {
...
exports.getOne = async (req, res) => {
  try {
    const product = await require('../../models/PvcMatsModels').findOne({ _id: req.params.id, category: '${category}' });
...
exports.delete = async (req, res) => {
  try {
    const product = await require('../../models/PvcMatsModels').findOneAndDelete({ _id: req.params.id, category: '${category}' });
    if (!product) return res.status(404).json({ error: 'Not found' });
...
for (const { rel } of controllers) {
  ...
  const importPath = `../controllers/roofer/${parts.length ? parts.join('/') + '/' : ''}${file}`;
  imports += `const ${controllerVar} = require('${importPath}');\n`;
  // Route: /api/roofer/<folders>/<controller>
  const routeBase = '/api/roofer/' + parts.map(toCamel).join('/') + (parts.length ? '/' : '') + toCamel(file.replace('Controller.js', ''));
