const Roofer = require('../../models/RooferModels');
 ...
 */
exports.createAluminiumSheet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Aluminium Sheet' };
    ...
    const product = new Roofer(productData);
    await product.save();
 ...
 */
exports.updateAluminiumSheet = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Roofer.findOneAndUpdate(
      { _id: req.params.id, category: 'Aluminium Sheet' },
      update,
      { new: true }
    );
...
exports.getAllAluminiumSheet = async (req, res) => {
  try {
    const products = await Roofer.find({ category: 'Aluminium Sheet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAluminiumSheet = async (req, res) => {
  try {
    const product = await Roofer.findOne({ _id: req.params.id, category: 'Aluminium Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAluminiumSheet = async (req, res) => {
  try {
    const product = await Roofer.findOneAndDelete({ _id: req.params.id, category: 'Aluminium Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
