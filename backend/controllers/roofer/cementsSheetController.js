 ...
 */
exports.createCementsSheet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Cements Sheet' };
 ...
 */
exports.updateCementsSheet = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Roofer.findOneAndUpdate(
      { _id: req.params.id, category: 'Cements Sheet' },
      update,
      { new: true }
    );
...
exports.getAllCementsSheet = async (req, res) => {
  try {
    const products = await Roofer.find({ category: 'Cements Sheet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCementsSheet = async (req, res) => {
  try {
    const product = await Roofer.findOne({ _id: req.params.id, category: 'Cements Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCementsSheet = async (req, res) => {
  try {
    const product = await Roofer.findOneAndDelete({ _id: req.params.id, category: 'Cements Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
