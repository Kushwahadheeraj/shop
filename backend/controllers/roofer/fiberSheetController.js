 ...
 */
exports.createFiberSheet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Fiber Sheet' };
 ...
 */
exports.updateFiberSheet = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Roofer.findOneAndUpdate(
      { _id: req.params.id, category: 'Fiber Sheet' },
      update,
      { new: true }
    );
...
exports.getAllFiberSheet = async (req, res) => {
  try {
    const products = await Roofer.find({ category: 'Fiber Sheet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFiberSheet = async (req, res) => {
  try {
    const product = await Roofer.findOne({ _id: req.params.id, category: 'Fiber Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFiberSheet = async (req, res) => {
  try {
    const product = await Roofer.findOneAndDelete({ _id: req.params.id, category: 'Fiber Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
