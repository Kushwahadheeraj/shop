...
exports.getAllMetal = async (req, res) => {
  try {
    const products = await Roofer.find({ category: 'Metal' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMetal = async (req, res) => {
  try {
    const product = await Roofer.findOne({ _id: req.params.id, category: 'Metal' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteMetal = async (req, res) => {
  try {
    const product = await Roofer.findOneAndDelete({ _id: req.params.id, category: 'Metal' });
    if (!product) return res.status(404).json({ error: 'Not found' });
