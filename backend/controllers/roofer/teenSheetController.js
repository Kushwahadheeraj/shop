 ...
 */
exports.createTeenSheet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Teen Sheet' };
 ...
 */
exports.updateTeenSheet = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Roofer.findOneAndUpdate(
      { _id: req.params.id, category: 'Teen Sheet' },
      update,
      { new: true }
    );
...
exports.getAllTeenSheet = async (req, res) => {
  try {
    const products = await Roofer.find({ category: 'Teen Sheet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTeenSheet = async (req, res) => {
  try {
    const product = await Roofer.findOne({ _id: req.params.id, category: 'Teen Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTeenSheet = async (req, res) => {
  try {
    const product = await Roofer.findOneAndDelete({ _id: req.params.id, category: 'Teen Sheet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
