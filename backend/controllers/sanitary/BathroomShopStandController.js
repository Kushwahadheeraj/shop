...
exports.createBathroomShopStand = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BathroomShopStand' };
...
exports.getAllBathroomShopStand = async (req, res) => {
  try {
...
exports.getBathroomShopStandById = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BathroomShopStand' });
    if (!product) {
...
exports.updateBathroomShopStand = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updatedProduct = await Sanitary.findOneAndUpdate(
      { _id: id, category: 'BathroomShopStand' },
      updateData,
      { new: true }
    );
...
exports.deleteBathroomShopStand = async (req, res) => {
  try {
    const deletedProduct = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BathroomShopStand' });
    if (!deletedProduct) {
      return res.status(404).json({ error: 'Product not found' });
    }
