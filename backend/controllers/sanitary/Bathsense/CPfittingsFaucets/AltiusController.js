...
exports.createAltius = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Altius' };
...
exports.getAllAltius = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Altius' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAltius = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Altius' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAltius = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Altius' });
