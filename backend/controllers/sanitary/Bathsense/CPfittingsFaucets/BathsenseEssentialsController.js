...
exports.createBathsenseEssentials = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BathsenseEssentials' };
...
exports.getAllBathsenseEssentials = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'BathsenseEssentials' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBathsenseEssentials = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BathsenseEssentials' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBathsenseEssentials = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BathsenseEssentials' });
