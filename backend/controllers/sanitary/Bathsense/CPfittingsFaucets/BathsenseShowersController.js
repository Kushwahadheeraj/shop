...
exports.createBathsenseShowers = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BathsenseShowers' };
...
exports.getAllBathsenseShowers = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'BathsenseShowers' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBathsenseShowers = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BathsenseShowers' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBathsenseShowers = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BathsenseShowers' });
