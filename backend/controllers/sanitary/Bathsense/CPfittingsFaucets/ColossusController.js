...
/**
 * Create a new Colossus product.
 */
exports.createColossus = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Colossus' };
...
/**
 * Update a Colossus product by ID.
 */
exports.updateColossus = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Colossus' },
      update,
      { new: true }
    );
...
exports.getAllColossus = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Colossus' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneColossus = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Colossus' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteColossus = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Colossus' });
