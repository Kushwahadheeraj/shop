...
exports.createColossus = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'bathsense/CPfittingsFaucets/Colossus' });
    await product.save();
...
exports.updateColossus = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'bathsense/CPfittingsFaucets/Colossus' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
