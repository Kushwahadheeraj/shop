...
exports.createInvictus = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'bathsense/CPfittingsFaucets/Invictus' });
    await product.save();
...
exports.updateInvictus = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'bathsense/CPfittingsFaucets/Invictus' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
