...
exports.createMaximus = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Maximus' };
...
exports.getAllMaximus = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Maximus' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMaximus = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Maximus' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteMaximus = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Maximus' });
