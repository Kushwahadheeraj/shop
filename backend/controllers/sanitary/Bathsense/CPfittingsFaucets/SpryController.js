...
exports.createSpry = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Spry' };
...
exports.getAllSpry = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Spry' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSpry = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Spry' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSpry = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Spry' });
