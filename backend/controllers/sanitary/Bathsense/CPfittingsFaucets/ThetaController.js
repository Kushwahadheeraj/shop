...
exports.createTheta = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'bathsense/CPfittingsFaucets/Theta' });
    await product.save();
...
exports.updateTheta = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'bathsense/CPfittingsFaucets/Theta' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
