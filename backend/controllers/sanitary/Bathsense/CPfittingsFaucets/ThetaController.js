...
/**
 * Create a new Theta product.
 */
exports.createTheta = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Theta' };
...
/**
 * Update a Theta product by ID.
 */
exports.updateTheta = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Theta' },
      update,
      { new: true }
    );
...
exports.getAllTheta = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Theta' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTheta = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Theta' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTheta = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Theta' });
