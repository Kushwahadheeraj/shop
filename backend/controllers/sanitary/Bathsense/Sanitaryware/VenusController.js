...
exports.createVenus = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Venus' };
...
exports.getAllVenus = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Venus' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneVenus = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Venus' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteVenus = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Venus' });
