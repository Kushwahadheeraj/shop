...
exports.createWaterCloset = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'waterCloset' };
...
exports.getAllWaterCloset = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'waterCloset' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWaterCloset = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'waterCloset' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWaterCloset = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'waterCloset' });
