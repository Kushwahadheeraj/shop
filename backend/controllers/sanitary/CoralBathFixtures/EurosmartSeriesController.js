...
/**
 * Create a new EurosmartSeries product.
 */
exports.createEurosmartSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'EurosmartSeries' };
...
/**
 * Update a EurosmartSeries product by ID.
 */
exports.updateEurosmartSeries = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'EurosmartSeries' },
      update,
      { new: true }
    );
...
exports.getAllEurosmartSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'EurosmartSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEurosmartSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'EurosmartSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEurosmartSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'EurosmartSeries' });
