...
exports.createEurosmartSeries = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'coralBathFixtures/EurosmartSeries' });
    await product.save();
...
exports.updateEurosmartSeries = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'coralBathFixtures/EurosmartSeries' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
