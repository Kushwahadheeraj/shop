...
exports.createNewSuperGlowSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'NewSuperGlowSeries' };
...
exports.getAllNewSuperGlowSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'NewSuperGlowSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNewSuperGlowSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'NewSuperGlowSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNewSuperGlowSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'NewSuperGlowSeries' });
