...
/**
 * Create a new TreemoSeries product.
 */
exports.createTreemoSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'TreemoSeries' };
...
/**
 * Update a TreemoSeries product by ID.
 */
exports.updateTreemoSeries = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'TreemoSeries' },
      update,
      { new: true }
    );
...
exports.getAllTreemoSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'TreemoSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTreemoSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'TreemoSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTreemoSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'TreemoSeries' });
