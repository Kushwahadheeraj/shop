...
exports.createAcrylicAccessories = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'AcrylicAccessories' };
...
exports.getAllAcrylicAccessories = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'AcrylicAccessories' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAcrylicAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'AcrylicAccessories' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAcrylicAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'AcrylicAccessories' });
