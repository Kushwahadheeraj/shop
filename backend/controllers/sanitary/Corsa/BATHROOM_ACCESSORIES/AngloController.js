...
exports.createAnglo = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Anglo' };
...
exports.getAllAnglo = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Anglo' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAnglo = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Anglo' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAnglo = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Anglo' });
