...
exports.createBudget = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BATHROOMACCESSORIES/Budget' });
    await product.save();
...
exports.updateBudget = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BATHROOMACCESSORIES/Budget' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
