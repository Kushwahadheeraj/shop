...
exports.createDolphin = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Dolphin' };
...
exports.getAllDolphin = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Dolphin' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDolphin = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Dolphin' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteDolphin = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Dolphin' });
