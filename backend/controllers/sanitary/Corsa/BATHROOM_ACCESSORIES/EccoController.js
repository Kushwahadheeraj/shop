...
exports.createEcco = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BATHROOMACCESSORIES/Ecco' });
    await product.save();
...
exports.updateEcco = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BATHROOMACCESSORIES/Ecco' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
