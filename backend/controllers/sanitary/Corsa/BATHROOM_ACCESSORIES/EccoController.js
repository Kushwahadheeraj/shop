...
exports.createEcco = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Ecco' };
...
exports.getAllEcco = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Ecco' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEcco = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Ecco' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEcco = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Ecco' });
