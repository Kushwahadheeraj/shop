...
/**
 * Create a new Keti product.
 */
exports.createKeti = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Keti' };
...
/**
 * Update a Keti product by ID.
 */
exports.updateKeti = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Keti' },
      update,
      { new: true }
    );
...
exports.getAllKeti = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Keti' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneKeti = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Keti' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteKeti = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Keti' });
