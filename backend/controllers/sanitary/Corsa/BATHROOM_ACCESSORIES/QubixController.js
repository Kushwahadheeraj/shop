...
/**
 * Create a new Qubix product.
 */
exports.createQubix = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Qubix' };
...
/**
 * Update a Qubix product by ID.
 */
exports.updateQubix = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Qubix' },
      update,
      { new: true }
    );
...
exports.getAllQubix = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Qubix' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneQubix = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Qubix' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteQubix = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Qubix' });
