...
exports.createQubix = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BATHROOMACCESSORIES/Qubix' });
    await product.save();
...
exports.updateQubix = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BATHROOMACCESSORIES/Qubix' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
