...
exports.createSquare = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Square' };
...
exports.getAllSquare = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Square' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSquare = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Square' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSquare = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Square' });
