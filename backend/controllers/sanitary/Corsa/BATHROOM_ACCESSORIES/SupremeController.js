...
exports.createSupreme = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Supreme' };
...
exports.getAllSupreme = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Supreme' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSupreme = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Supreme' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSupreme = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Supreme' });
