...
exports.createAlmond = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Almond' };
...
exports.getAllAlmond = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Almond' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAlmond = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Almond' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAlmond = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Almond' });
