...
exports.createAlmond = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BathroomFaucets/Almond' });
    await product.save();
...
exports.updateAlmond = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BathroomFaucets/Almond' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
