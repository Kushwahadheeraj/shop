...
exports.createArrow = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Arrow' };
...
exports.getAllArrow = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Arrow' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneArrow = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Arrow' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteArrow = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Arrow' });
