...
exports.createBold = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Bold' };
...
exports.getAllBold = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Bold' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBold = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Bold' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBold = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Bold' });
