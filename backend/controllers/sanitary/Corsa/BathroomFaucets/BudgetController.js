...
exports.createBudget = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Budget' };
...
exports.getAllBudget = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Budget' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBudget = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Budget' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBudget = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Budget' });
