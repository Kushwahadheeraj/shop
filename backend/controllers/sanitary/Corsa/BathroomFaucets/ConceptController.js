...
exports.createConcept = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Concept' };
...
exports.getAllConcept = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Concept' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneConcept = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Concept' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteConcept = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Concept' });
