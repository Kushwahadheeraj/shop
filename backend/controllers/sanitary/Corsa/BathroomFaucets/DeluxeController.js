...
exports.createDeluxe = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Deluxe' };
...
exports.getAllDeluxe = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Deluxe' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDeluxe = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Deluxe' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteDeluxe = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Deluxe' });
