...
exports.createEeco = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Eeco' };
...
exports.getAllEeco = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Eeco' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEeco = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Eeco' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEeco = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Eeco' });
