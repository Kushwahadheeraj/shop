...
exports.createExpert = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BathroomFaucets/Expert' });
    await product.save();
...
exports.updateExpert = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BathroomFaucets/Expert' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
