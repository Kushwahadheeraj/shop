...
exports.createExpert = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Expert' };
...
exports.updateExpert = async (req, res) => {
  try {
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Expert' },
      update,
      { new: true }
    );
...
exports.getAllExpert = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Expert' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneExpert = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Expert' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteExpert = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Expert' });
