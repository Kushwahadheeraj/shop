...
exports.createFlorence = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Florence' };
...
exports.getAllFlorence = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Florence' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFlorence = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Florence' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFlorence = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Florence' });
