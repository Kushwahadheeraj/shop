...
/**
 * Create a new GlassBowlFaucet product.
 */
exports.createGlassBowlFaucet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'GlassBowlFaucet' };
...
/**
 * Update a GlassBowlFaucet product by ID.
 */
exports.updateGlassBowlFaucet = async (req, res) => {
  try {
    let update = { ...req.body };
...
exports.getAllGlassBowlFaucet = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'GlassBowlFaucet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneGlassBowlFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'GlassBowlFaucet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteGlassBowlFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'GlassBowlFaucet' });
