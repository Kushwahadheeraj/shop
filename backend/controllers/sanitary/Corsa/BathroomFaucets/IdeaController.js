...
exports.createIdea = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Idea' };
...
exports.getAllIdea = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Idea' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneIdea = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Idea' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteIdea = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Idea' });
