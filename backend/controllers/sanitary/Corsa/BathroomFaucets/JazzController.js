...
exports.createJazz = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Jazz' };
...
exports.getAllJazz = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Jazz' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneJazz = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Jazz' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteJazz = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Jazz' });
