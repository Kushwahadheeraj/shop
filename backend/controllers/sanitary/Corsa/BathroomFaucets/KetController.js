...
exports.createKet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Ket' };
...
exports.getAllKet = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Ket' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneKet = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Ket' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteKet = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Ket' });
