...
exports.createMilano = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Milano' };
...
exports.getAllMilano = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Milano' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMilano = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Milano' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteMilano = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Milano' });
