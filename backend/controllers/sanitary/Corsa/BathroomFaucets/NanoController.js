...
exports.createNano = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Nano' };
...
exports.getAllNano = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Nano' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNano = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Nano' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNano = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Nano' });
