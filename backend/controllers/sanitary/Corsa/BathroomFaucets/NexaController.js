...
exports.createNexa = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Nexa' };
...
exports.getAllNexa = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Nexa' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNexa = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Nexa' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNexa = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Nexa' });
