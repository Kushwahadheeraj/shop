...
exports.createNiagra = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Niagra' };
...
exports.getAllNiagra = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Niagra' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNiagra = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Niagra' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNiagra = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Niagra' });
