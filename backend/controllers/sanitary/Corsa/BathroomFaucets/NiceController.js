...
exports.createNice = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Nice' };
...
exports.getAllNice = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Nice' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNice = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Nice' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNice = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Nice' });
