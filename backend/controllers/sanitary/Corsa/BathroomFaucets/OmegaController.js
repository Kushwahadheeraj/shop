...
exports.createOmega = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Omega' };
...
exports.getAllOmega = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Omega' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneOmega = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Omega' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteOmega = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Omega' });
