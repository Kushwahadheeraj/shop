...
exports.createPassion = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Passion' };
...
exports.getAllPassion = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Passion' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePassion = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Passion' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePassion = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Passion' });
