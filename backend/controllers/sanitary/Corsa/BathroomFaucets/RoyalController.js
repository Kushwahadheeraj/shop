...
exports.createRoyal = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Royal' };
...
exports.getAllRoyal = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Royal' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRoyal = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Royal' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRoyal = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Royal' });
