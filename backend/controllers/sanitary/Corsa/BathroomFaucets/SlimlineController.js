...
/**
 * Create a new Slimline product.
 */
exports.createSlimline = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Slimline' };
...
/**
 * Update a Slimline product by ID.
 */
exports.updateSlimline = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Slimline' },
      update,
      { new: true }
    );
...
exports.getAllSlimline = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Slimline' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSlimline = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Slimline' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSlimline = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Slimline' });
