...
exports.createSlimline = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BathroomFaucets/Slimline' });
    await product.save();
...
exports.updateSlimline = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BathroomFaucets/Slimline' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
