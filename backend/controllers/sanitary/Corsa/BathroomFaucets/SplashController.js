...
exports.createSplash = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/BathroomFaucets/Splash' });
    await product.save();
...
exports.updateSplash = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/BathroomFaucets/Splash' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
