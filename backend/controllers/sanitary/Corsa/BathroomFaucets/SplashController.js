...
exports.createSplash = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Splash' };
...
exports.updateSplash = async (req, res) => {
  try {
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Splash' },
      update,
      { new: true }
    );
...
exports.getAllSplash = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Splash' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSplash = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Splash' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSplash = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Splash' });
