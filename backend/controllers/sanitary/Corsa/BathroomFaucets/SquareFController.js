...
exports.createSquareF = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SquareF' };
...
exports.getAllSquareF = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SquareF' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSquareF = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SquareF' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSquareF = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SquareF' });
