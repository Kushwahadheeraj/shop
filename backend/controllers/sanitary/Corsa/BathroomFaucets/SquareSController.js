...
exports.createSquareS = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SquareS' };
...
exports.getAllSquareS = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SquareS' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSquareS = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SquareS' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSquareS = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SquareS' });
