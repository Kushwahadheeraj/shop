...
exports.createSuper = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Super' };
...
exports.getAllSuper = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Super' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSuper = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Super' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSuper = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Super' });
