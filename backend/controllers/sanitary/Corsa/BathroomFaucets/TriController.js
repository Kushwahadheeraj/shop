...
exports.createTri = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Tri' };
...
exports.getAllTri = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Tri' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTri = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Tri' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTri = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Tri' });
