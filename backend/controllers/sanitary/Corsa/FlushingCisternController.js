...
exports.createFlushingCistern = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/FlushingCistern' });
    await product.save();
...
exports.updateFlushingCistern = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/FlushingCistern' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
