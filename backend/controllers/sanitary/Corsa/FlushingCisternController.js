...
/**
 * Create a new FlushingCistern product.
 */
exports.createFlushingCistern = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'FlushingCistern' };
...
/**
 * Update a FlushingCistern product by ID.
 */
exports.updateFlushingCistern = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'FlushingCistern' },
      update,
      { new: true }
    );
...
exports.getAllFlushingCistern = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'FlushingCistern' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFlushingCistern = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'FlushingCistern' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFlushingCistern = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'FlushingCistern' });
