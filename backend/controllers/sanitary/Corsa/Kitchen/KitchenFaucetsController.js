...
exports.createKitchenFaucets = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'KitchenFaucets' };
...
exports.getAllKitchenFaucets = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'KitchenFaucets' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneKitchenFaucets = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'KitchenFaucets' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteKitchenFaucets = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'KitchenFaucets' });
