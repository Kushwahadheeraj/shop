...
exports.createKitchenSink = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'KitchenSink' };
...
exports.getAllKitchenSink = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'KitchenSink' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneKitchenSink = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'KitchenSink' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteKitchenSink = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'KitchenSink' });
