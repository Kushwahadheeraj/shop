...
exports.createBallValves = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BallValves' };
...
exports.getAllBallValves = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'BallValves' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBallValves = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BallValves' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBallValves = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BallValves' });
