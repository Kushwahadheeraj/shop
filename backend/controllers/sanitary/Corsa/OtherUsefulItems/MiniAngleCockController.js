...
exports.createMiniAngleCock = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/OtherUsefulItems/MiniAngleCock' });
    await product.save();
...
exports.updateMiniAngleCock = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/OtherUsefulItems/MiniAngleCock' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
