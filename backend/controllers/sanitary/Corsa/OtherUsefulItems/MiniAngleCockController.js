...
/**
 * Create a new MiniAngleCock product.
 */
exports.createMiniAngleCock = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'MiniAngleCock' };
...
/**
 * Update a MiniAngleCock product by ID.
 */
exports.updateMiniAngleCock = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'MiniAngleCock' },
      update,
      { new: true }
    );
...
exports.getAllMiniAngleCock = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'MiniAngleCock' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMiniAngleCock = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'MiniAngleCock' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteMiniAngleCock = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'MiniAngleCock' });
