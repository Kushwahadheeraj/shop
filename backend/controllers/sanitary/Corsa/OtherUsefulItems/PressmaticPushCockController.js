...
exports.createPressmaticPushCock = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PressmaticPushCock' };
...
exports.getAllPressmaticPushCock = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'PressmaticPushCock' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePressmaticPushCock = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PressmaticPushCock' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePressmaticPushCock = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PressmaticPushCock' });
