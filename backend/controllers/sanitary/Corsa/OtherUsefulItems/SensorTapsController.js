...
/**
 * Create a new SensorTaps product.
 */
exports.createSensorTaps = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SensorTaps' };
...
/**
 * Update a SensorTaps product by ID.
 */
exports.updateSensorTaps = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'SensorTaps' },
      update,
      { new: true }
    );
...
exports.getAllSensorTaps = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SensorTaps' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSensorTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SensorTaps' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSensorTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SensorTaps' });
