...
/**
 * Create a new SoapDispenser product.
 */
exports.createSoapDispenser = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SoapDispenser' };
...
/**
 * Update a SoapDispenser product by ID.
 */
exports.updateSoapDispenser = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'SoapDispenser' },
      update,
      { new: true }
    );
...
exports.getAllSoapDispenser = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SoapDispenser' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSoapDispenser = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SoapDispenser' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSoapDispenser = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SoapDispenser' });
