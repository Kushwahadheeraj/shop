...
exports.createSoapDispenser = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/OtherUsefulItems/SoapDispenser' });
    await product.save();
...
exports.updateSoapDispenser = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/OtherUsefulItems/SoapDispenser' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
