...
exports.createOverheadShower = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'OverheadShower' };
...
exports.getAllOverheadShower = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'OverheadShower' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneOverheadShower = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'OverheadShower' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteOverheadShower = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'OverheadShower' });
