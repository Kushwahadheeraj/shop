...
exports.createRainShower = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'RainShower' };
...
exports.getAllRainShower = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'RainShower' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRainShower = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'RainShower' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRainShower = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'RainShower' });
