...
exports.createRainShower = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'corsa/SHOWERS/RainShower' });
    await product.save();
...
exports.updateRainShower = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'corsa/SHOWERS/RainShower' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
