...
exports.createRainShower = async (req, res) => {
  try {
    ...
    const product = new Sanitary({ ...req.body, photos: photoUrls, category: 'RainShower' });
    await product.save();
...
exports.getAllRainShower = async (req, res) => {
  try {
    const products = await require('../../models/SanitaryModels').find({ category: 'RainShower' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRainShower = async (req, res) => {
  try {
    const product = await require('../../models/SanitaryModels').findOne({ _id: req.params.id, category: 'RainShower' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRainShower = async (req, res) => {
  try {
    const product = await require('../../models/SanitaryModels').findOneAndDelete({ _id: req.params.id, category: 'RainShower' });
    if (!product) return res.status(404).json({ error: 'Not found' });
