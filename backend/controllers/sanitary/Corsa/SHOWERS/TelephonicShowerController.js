...
exports.createTelephonicShower = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'TelephonicShower' };
...
exports.getAllTelephonicShower = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'TelephonicShower' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTelephonicShower = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'TelephonicShower' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTelephonicShower = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'TelephonicShower' });
