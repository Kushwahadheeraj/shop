...
exports.createSeries1Croma = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Series1Croma' };
...
exports.getAllSeries1Croma = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Series1Croma' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSeries1Croma = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Series1Croma' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSeries1Croma = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Series1Croma' });
