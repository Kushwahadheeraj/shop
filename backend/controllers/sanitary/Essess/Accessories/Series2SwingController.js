...
/**
 * Create a new Series2Swing product.
 */
exports.createSeries2Swing = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Series2Swing' };
...
/**
 * Update a Series2Swing product by ID.
 */
exports.updateSeries2Swing = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Series2Swing' },
      update,
      { new: true }
    );
...
exports.getAllSeries2Swing = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Series2Swing' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSeries2Swing = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Series2Swing' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSeries2Swing = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Series2Swing' });
