...
exports.createSeries3Tarim = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Series3Tarim' };
...
exports.getAllSeries3Tarim = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Series3Tarim' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSeries3Tarim = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Series3Tarim' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSeries3Tarim = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Series3Tarim' });
