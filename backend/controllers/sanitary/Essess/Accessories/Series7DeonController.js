...
exports.createSeries7Deon = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Series7Deon' };
...
exports.getAllSeries7Deon = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Series7Deon' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSeries7Deon = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Series7Deon' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSeries7Deon = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Series7Deon' });
