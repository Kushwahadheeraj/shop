...
exports.createSeries7Deon = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'essess/Accessories/Series7Deon' });
    await product.save();
...
exports.updateSeries7Deon = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'essess/Accessories/Series7Deon' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
