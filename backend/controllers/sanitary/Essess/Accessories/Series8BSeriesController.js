...
exports.createSeries8BSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Series8BSeries' };
...
exports.getAllSeries8BSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Series8BSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSeries8BSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Series8BSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSeries8BSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Series8BSeries' });
