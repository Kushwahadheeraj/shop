...
exports.createSeries8BSeries = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'essess/Accessories/Series8BSeries' });
    await product.save();
...
exports.updateSeries8BSeries = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'essess/Accessories/Series8BSeries' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
