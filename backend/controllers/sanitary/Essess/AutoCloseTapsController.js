...
exports.createAutoCloseTaps = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'AutoCloseTaps' };
...
exports.getAllAutoCloseTaps = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'AutoCloseTaps' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAutoCloseTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'AutoCloseTaps' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAutoCloseTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'AutoCloseTaps' });
