...
exports.createCelato = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Celato' };
...
exports.getAllCelato = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Celato' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCelato = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Celato' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCelato = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Celato' });
