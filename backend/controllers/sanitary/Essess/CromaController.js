...
exports.createCroma = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Croma' };
...
exports.getAllCroma = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Croma' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCroma = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Croma' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCroma = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Croma' });
