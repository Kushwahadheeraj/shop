...
exports.createCruzo = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Cruzo' };
...
exports.getAllCruzo = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Cruzo' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCruzo = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Cruzo' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCruzo = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Cruzo' });
