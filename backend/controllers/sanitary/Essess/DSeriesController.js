...
exports.createDSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'DSeries' };
...
exports.getAllDSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'DSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'DSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteDSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'DSeries' });
