...
exports.createDeon = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Deon' };
...
exports.getAllDeon = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Deon' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDeon = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Deon' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteDeon = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Deon' });
