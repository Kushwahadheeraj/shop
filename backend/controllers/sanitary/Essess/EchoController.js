...
exports.createEcho = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Echo' };
...
exports.getAllEcho = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Echo' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEcho = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Echo' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEcho = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Echo' });
