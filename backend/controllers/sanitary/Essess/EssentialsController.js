...
exports.createEssentials = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Essentials' };
...
exports.getAllEssentials = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Essentials' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEssentials = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Essentials' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEssentials = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Essentials' });
