...
/**
 * Create a new HotelierSeries product.
 */
exports.createHotelierSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'HotelierSeries' };
...
/**
 * Update a HotelierSeries product by ID.
 */
exports.updateHotelierSeries = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'HotelierSeries' },
      update,
      { new: true }
    );
...
exports.getAllHotelierSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'HotelierSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHotelierSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'HotelierSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteHotelierSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'HotelierSeries' });
