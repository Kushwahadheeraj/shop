...
exports.createLabTaps = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'LabTaps' };
...
exports.getAllLabTaps = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'LabTaps' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneLabTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'LabTaps' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteLabTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'LabTaps' });
