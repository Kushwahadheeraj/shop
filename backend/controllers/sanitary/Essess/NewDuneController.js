...
/**
 * Create a new NewDune product.
 */
exports.createNewDune = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'NewDune' };
...
/**
 * Update a NewDune product by ID.
 */
exports.updateNewDune = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'NewDune' },
      update,
      { new: true }
    );
...
exports.getAllNewDune = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'NewDune' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNewDune = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'NewDune' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNewDune = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'NewDune' });
