...
exports.createNewXess = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'NewXess' };
...
exports.getAllNewXess = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'NewXess' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneNewXess = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'NewXess' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteNewXess = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'NewXess' });
