...
exports.createQuadra = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Quadra' };
...
exports.getAllQuadra = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Quadra' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneQuadra = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Quadra' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteQuadra = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Quadra' });
