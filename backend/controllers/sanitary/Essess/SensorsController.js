...
exports.createSensors = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Sensors' };
...
/**
 * Update a Sensors product by ID.
 */
exports.updateSensors = async (req, res) => {
  try {
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'Sensors' },
      update,
      { new: true }
    );
...
exports.getAllSensors = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Sensors' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSensors = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Sensors' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSensors = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Sensors' });
