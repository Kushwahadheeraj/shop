...
/**
 * Create a new RainfallShowers product.
 */
exports.createRainfallShowers = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'RainfallShowers' };
...
/**
 * Update a RainfallShowers product by ID.
 */
exports.updateRainfallShowers = async (req, res) => {
  try {
    let update = { ...req.body };
    ...
    const product = await Sanitary.findOneAndUpdate(
      { _id: req.params.id, category: 'RainfallShowers' },
      update,
      { new: true }
    );
...
exports.getAllRainfallShowers = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'RainfallShowers' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRainfallShowers = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'RainfallShowers' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRainfallShowers = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'RainfallShowers' });
