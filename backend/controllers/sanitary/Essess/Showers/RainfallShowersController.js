...
exports.createRainfallShowers = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'essess/Showers/RainfallShowers' });
    await product.save();
...
exports.updateRainfallShowers = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'essess/Showers/RainfallShowers' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
