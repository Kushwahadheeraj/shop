...
exports.createShowerArms = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ShowerArms' };
...
exports.getAllShowerArms = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ShowerArms' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneShowerArms = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ShowerArms' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteShowerArms = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ShowerArms' });
