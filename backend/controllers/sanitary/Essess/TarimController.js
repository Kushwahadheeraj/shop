...
exports.createTarim = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Tarim' };
...
exports.getAllTarim = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Tarim' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTarim = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Tarim' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTarim = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Tarim' });
