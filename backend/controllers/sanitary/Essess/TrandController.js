...
exports.createTrand = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Trand' };
...
exports.getAllTrand = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Trand' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTrand = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Trand' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTrand = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Trand' });
