...
exports.createAddOn = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'AddOn' };
...
exports.getAllAddOn = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'AddOn' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAddOn = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'AddOn' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAddOn = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'AddOn' });
