...
exports.createBathTub = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BathTub' };
...
exports.getAllBathTub = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'BathTub' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBathTub = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BathTub' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBathTub = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BathTub' });
