...
exports.createCisterns = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Cisterns' };
...
exports.getAllCisterns = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Cisterns' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCisterns = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Cisterns' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCisterns = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Cisterns' });
