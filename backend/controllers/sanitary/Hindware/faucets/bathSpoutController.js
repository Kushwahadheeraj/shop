...
exports.createBathSpout = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'hindware/faucets/bathSpout' });
    await product.save();
...
exports.updateBathSpout = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'hindware/faucets/bathSpout' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
