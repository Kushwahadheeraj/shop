...
/**
 * Create a new BathSpout product.
 */
exports.createBathSpout = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BathSpout' };
...
/**
 * Update a BathSpout product by ID.
 */
exports.updateBathSpout = async (req, res) => {
  try {
    let update = { ...req.body };
...
exports.getAllBathSpout = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'BathSpout' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBathSpout = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BathSpout' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBathSpout = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BathSpout' });
