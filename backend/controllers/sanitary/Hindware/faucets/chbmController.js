...
exports.createChbm = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Chbm' };
...
exports.getAllChbm = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Chbm' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneChbm = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Chbm' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteChbm = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Chbm' });
