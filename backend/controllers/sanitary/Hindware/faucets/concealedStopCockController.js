...
exports.createConcealedStopCock = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ConcealedStopCock' };
...
exports.getAllConcealedStopCock = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ConcealedStopCock' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneConcealedStopCock = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ConcealedStopCock' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteConcealedStopCock = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ConcealedStopCock' });
