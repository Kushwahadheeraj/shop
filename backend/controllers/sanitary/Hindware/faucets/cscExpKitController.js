...
exports.createCscExpKit = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'CscExpKit' };
...
exports.getAllCscExpKit = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'CscExpKit' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCscExpKit = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'CscExpKit' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCscExpKit = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'CscExpKit' });
