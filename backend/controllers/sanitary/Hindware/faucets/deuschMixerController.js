...
exports.createDeuschMixer = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'DeuschMixer' };
...
exports.getAllDeuschMixer = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'DeuschMixer' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDeuschMixer = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'DeuschMixer' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteDeuschMixer = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'DeuschMixer' });
