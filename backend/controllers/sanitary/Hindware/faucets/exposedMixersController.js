...
exports.createExposedMixers = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'hindware/faucets/exposedMixers' });
    await product.save();
...
exports.updateExposedMixers = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'hindware/faucets/exposedMixers' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
