...
exports.createMedicalSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'MedicalSeries' };
...
exports.getAllMedicalSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'MedicalSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMedicalSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'MedicalSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteMedicalSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'MedicalSeries' });
