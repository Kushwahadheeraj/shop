...
exports.createMixerFaucet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'MixerFaucet' };
...
exports.getAllMixerFaucet = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'MixerFaucet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneMixerFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'MixerFaucet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteMixerFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'MixerFaucet' });
