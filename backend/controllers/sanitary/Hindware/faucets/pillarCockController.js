...
exports.createPillarCock = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PillarCock' };
...
exports.getAllPillarCock = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'PillarCock' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePillarCock = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PillarCock' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePillarCock = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PillarCock' });
