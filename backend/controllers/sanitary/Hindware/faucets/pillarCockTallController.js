...
exports.createPillarCockTall = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PillarCockTall' };
...
exports.getAllPillarCockTall = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'PillarCockTall' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePillarCockTall = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PillarCockTall' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePillarCockTall = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PillarCockTall' });
