...
exports.createPillarFaucet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PillarFaucet' };
...
exports.getAllPillarFaucet = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'PillarFaucet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePillarFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PillarFaucet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePillarFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PillarFaucet' });
