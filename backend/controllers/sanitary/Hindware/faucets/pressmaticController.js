...
exports.createPressmatic = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Pressmatic' };
...
exports.getAllPressmatic = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Pressmatic' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePressmatic = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Pressmatic' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePressmatic = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Pressmatic' });
