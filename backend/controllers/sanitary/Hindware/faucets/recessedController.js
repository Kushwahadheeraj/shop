...
exports.createRecessed = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Recessed' };
...
exports.getAllRecessed = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Recessed' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRecessed = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Recessed' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRecessed = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Recessed' });
