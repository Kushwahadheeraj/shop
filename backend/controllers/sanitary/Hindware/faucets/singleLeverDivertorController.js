...
exports.createSingleLeverDivertor = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SingleLeverDivertor' };
...
exports.getAllSingleLeverDivertor = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SingleLeverDivertor' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSingleLeverDivertor = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SingleLeverDivertor' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSingleLeverDivertor = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SingleLeverDivertor' });
