...
/**
 * Create a new SinkCock product.
 */
exports.createSinkCock = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SinkCock' };
...
/**
 * Update a SinkCock product by ID.
 */
exports.updateSinkCock = async (req, res) => {
  try {
    let update = { ...req.body };
...
exports.getAllSinkCock = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SinkCock' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSinkCock = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SinkCock' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSinkCock = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SinkCock' });
