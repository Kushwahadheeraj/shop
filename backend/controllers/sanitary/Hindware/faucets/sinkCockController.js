...
exports.createSinkCock = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'hindware/faucets/sinkCock' });
    await product.save();
...
exports.updateSinkCock = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'hindware/faucets/sinkCock' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
