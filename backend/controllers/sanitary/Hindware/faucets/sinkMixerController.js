...
exports.createSinkMixer = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SinkMixer' };
...
exports.getAllSinkMixer = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SinkMixer' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSinkMixer = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SinkMixer' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSinkMixer = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SinkMixer' });
