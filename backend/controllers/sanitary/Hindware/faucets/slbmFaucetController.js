...
exports.createSlbmFaucet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SlbmFaucet' };
...
exports.getAllSlbmFaucet = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SlbmFaucet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSlbmFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SlbmFaucet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSlbmFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SlbmFaucet' });
