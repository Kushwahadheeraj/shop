...
exports.createSlbmFaucetTall = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SlbmFaucetTall' };
...
exports.getAllSlbmFaucetTall = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SlbmFaucetTall' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSlbmFaucetTall = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SlbmFaucetTall' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSlbmFaucetTall = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SlbmFaucetTall' });
