...
exports.createWallMixer = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'WallMixer' };
...
exports.getAllWallMixer = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'WallMixer' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWallMixer = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'WallMixer' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWallMixer = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'WallMixer' });
