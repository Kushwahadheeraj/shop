...
exports.createRainShowers = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'RainShowers' };
...
exports.getAllRainShowers = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'RainShowers' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRainShowers = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'RainShowers' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRainShowers = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'RainShowers' });
