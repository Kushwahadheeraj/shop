...
exports.createWaterClosets = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'hindware/waterClosets' });
    await product.save();
...
exports.updateWaterClosets = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'hindware/waterClosets' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
