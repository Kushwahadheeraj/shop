...
/**
 * Create a new waterClosets product.
 */
exports.createWaterClosets = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'waterClosets' };
...
/**
 * Update a waterClosets product by ID.
 */
exports.updateWaterClosets = async (req, res) => {
  try {
    let update = { ...req.body };
...
exports.getAllWaterClosets = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'waterClosets' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWaterClosets = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'waterClosets' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWaterClosets = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'waterClosets' });
