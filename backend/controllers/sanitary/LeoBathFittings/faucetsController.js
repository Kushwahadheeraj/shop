...
exports.createFaucets = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Faucets' };
...
exports.getAllFaucets = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Faucets' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFaucets = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Faucets' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFaucets = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Faucets' });
