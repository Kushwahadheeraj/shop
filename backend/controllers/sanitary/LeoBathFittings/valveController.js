...
exports.createValve = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Valve' };
...
exports.getAllValve = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Valve' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneValve = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Valve' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteValve = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Valve' });
