...
/**
 * Create a new PTMTToti product.
 */
exports.createPTMTToti = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PTMTToti' };
...
/**
 * Get all PTMTToti products.
 */
exports.getAllPTMTToti = async (req, res) => {
  try {
...
/**
 * Get a single PTMTToti product by ID.
 */
exports.getPTMTTotiById = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PTMTToti' });
    if (!product) {
...
/**
 * Update a PTMTToti product by ID.
 */
exports.updatePTMTToti = async (req, res) => {
  try {
    const { id } = req.params;
...
/**
 * Delete a PTMTToti product by ID.
 */
exports.deletePTMTToti = async (req, res) => {
  try {
    const deletedProduct = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PTMTToti' });
    if (!deletedProduct) {
      return res.status(404).json({ error: 'Product not found' });
    }
