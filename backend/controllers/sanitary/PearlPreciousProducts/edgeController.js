...
exports.createEdge = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Edge' };
...
exports.getAllEdge = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Edge' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEdge = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Edge' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEdge = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Edge' });
