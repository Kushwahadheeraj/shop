...
exports.createSinkWashBasinJali = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SinkWashBasinJali' };
...
exports.getAllSinkWashBasinJali = async (req, res) => {
  try {
...
exports.getSinkWashBasinJaliById = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SinkWashBasinJali' });
    if (!product) {
...
exports.updateSinkWashBasinJali = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updatedProduct = await Sanitary.findOneAndUpdate(
      { _id: id, category: 'SinkWashBasinJali' },
      updateData,
      { new: true }
    );
...
exports.deleteSinkWashBasinJali = async (req, res) => {
  try {
    const deletedProduct = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SinkWashBasinJali' });
    if (!deletedProduct) {
      return res.status(404).json({ error: 'Product not found' });
    }
