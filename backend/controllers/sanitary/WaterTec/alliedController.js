...
exports.createAllied = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Allied' };
...
exports.getAllAllied = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Allied' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAllied = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Allied' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAllied = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Allied' });
