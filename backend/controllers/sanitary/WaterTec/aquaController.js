...
exports.createAqua = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Aqua' };
...
exports.getAllAqua = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Aqua' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAqua = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Aqua' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAqua = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Aqua' });
