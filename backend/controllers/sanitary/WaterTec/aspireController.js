...
exports.createAspire = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Aspire' };
...
exports.getAllAspire = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Aspire' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAspire = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Aspire' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAspire = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Aspire' });
