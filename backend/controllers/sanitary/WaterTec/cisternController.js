...
exports.createCistern = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Cistern' };
...
exports.getAllCistern = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Cistern' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneCistern = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Cistern' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteCistern = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Cistern' });
