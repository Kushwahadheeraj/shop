...
exports.createConcealedCistern = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ConcealedCistern' };
...
exports.getAllConcealedCistern = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ConcealedCistern' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneConcealedCistern = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ConcealedCistern' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteConcealedCistern = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ConcealedCistern' });
