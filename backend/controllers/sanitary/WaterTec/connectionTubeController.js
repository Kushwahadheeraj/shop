...
exports.createConnectionTube = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ConnectionTube' };
...
exports.getAllConnectionTube = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ConnectionTube' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneConnectionTube = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ConnectionTube' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteConnectionTube = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ConnectionTube' });
