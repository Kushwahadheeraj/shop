...
exports.createEbony = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Ebony' };
...
exports.getAllEbony = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Ebony' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEbony = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Ebony' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEbony = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Ebony' });
