...
exports.createEco = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Eco' };
...
exports.getAllEco = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Eco' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEco = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Eco' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEco = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Eco' });
