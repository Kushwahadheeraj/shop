...
exports.createEva = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Eva' };
...
exports.getAllEva = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Eva' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEva = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Eva' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEva = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Eva' });
