...
exports.createFlora = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Flora' };
...
exports.getAllFlora = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Flora' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFlora = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Flora' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFlora = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Flora' });
