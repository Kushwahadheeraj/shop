...
exports.createHealthFaucets = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'waterTec/healthFaucets' });
    await product.save();
...
exports.updateHealthFaucets = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'waterTec/healthFaucets' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
