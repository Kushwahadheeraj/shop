...
exports.createQuattro = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Quattro' };
...
exports.getAllQuattro = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Quattro' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneQuattro = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Quattro' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteQuattro = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Quattro' });
