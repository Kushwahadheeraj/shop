...
exports.createTSeriesAlt = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'TSeriesAlt' };
...
exports.getAllTSeriesAlt = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'TSeriesAlt' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTSeriesAlt = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'TSeriesAlt' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTSeriesAlt = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'TSeriesAlt' });
