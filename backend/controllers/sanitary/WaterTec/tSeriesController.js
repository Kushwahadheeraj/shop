...
exports.createTSeries = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'TSeries' };
...
exports.getAllTSeries = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'TSeries' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'TSeries' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTSeries = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'TSeries' });
