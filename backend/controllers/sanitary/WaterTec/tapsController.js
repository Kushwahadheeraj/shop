...
exports.createTaps = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Taps' };
...
exports.getAllTaps = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Taps' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Taps' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteTaps = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Taps' });
