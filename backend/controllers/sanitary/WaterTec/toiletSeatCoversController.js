...
exports.createToiletSeatCovers = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ToiletSeatCovers' };
...
exports.getAllToiletSeatCovers = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ToiletSeatCovers' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneToiletSeatCovers = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ToiletSeatCovers' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteToiletSeatCovers = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ToiletSeatCovers' });
