...
exports.createValves = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Valves' };
...
exports.getAllValves = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Valves' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneValves = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Valves' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteValves = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Valves' });
