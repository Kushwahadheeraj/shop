...
exports.createAccessories = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Accessories' };
...
exports.getAllAccessories = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Accessories' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Accessories' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Accessories' });
