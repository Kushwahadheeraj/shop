...
exports.createAria = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Aria' };
...
exports.getAllAria = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Aria' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAria = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Aria' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAria = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Aria' });
