...
exports.createDell = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Dell' };
...
exports.getAllDell = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Dell' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneDell = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Dell' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteDell = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Dell' });
