...
exports.createEvoque = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Evoque' };
...
exports.getAllEvoque = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Evoque' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEvoque = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Evoque' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEvoque = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Evoque' });
