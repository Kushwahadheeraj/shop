...
exports.createHandShowers = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'waterman/handShowers' });
    await product.save();
...
exports.updateHandShowers = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'waterman/handShowers' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
