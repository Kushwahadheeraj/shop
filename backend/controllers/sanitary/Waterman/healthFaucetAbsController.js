...
exports.createHealthFaucetAbs = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'HealthFaucetAbs' };
...
exports.getAllHealthFaucetAbs = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'HealthFaucetAbs' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHealthFaucetAbs = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'HealthFaucetAbs' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteHealthFaucetAbs = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'HealthFaucetAbs' });
