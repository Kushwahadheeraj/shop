...
exports.createHealthFaucetsBrass = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'HealthFaucetsBrass' };
...
exports.getAllHealthFaucetsBrass = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'HealthFaucetsBrass' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHealthFaucetsBrass = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'HealthFaucetsBrass' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteHealthFaucetsBrass = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'HealthFaucetsBrass' });
