...
exports.createIkon = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Ikon' };
...
exports.getAllIkon = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Ikon' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneIkon = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Ikon' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteIkon = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Ikon' });
