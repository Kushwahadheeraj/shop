...
exports.createRoman = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Roman' };
...
exports.getAllRoman = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Roman' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneRoman = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Roman' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteRoman = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Roman' });
