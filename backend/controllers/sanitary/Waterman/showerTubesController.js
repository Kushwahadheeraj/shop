...
exports.createShowerTubes = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ShowerTubes' };
...
exports.getAllShowerTubes = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ShowerTubes' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneShowerTubes = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ShowerTubes' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteShowerTubes = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ShowerTubes' });
