...
exports.createWallShowersWithArm = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'WallShowersWithArm' };
...
exports.getAllWallShowersWithArm = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'WallShowersWithArm' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWallShowersWithArm = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'WallShowersWithArm' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWallShowersWithArm = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'WallShowersWithArm' });
