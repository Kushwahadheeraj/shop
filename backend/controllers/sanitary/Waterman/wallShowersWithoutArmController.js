...
/**
 * Create a new WallShowersWithoutArm product.
 */
exports.createWallShowersWithoutArm = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'WallShowersWithoutArm' };
...
/**
 * Update a WallShowersWithoutArm product by ID.
 */
exports.updateWallShowersWithoutArm = async (req, res) => {
  try {
    let update = { ...req.body };
...
exports.getAllWallShowersWithoutArm = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'WallShowersWithoutArm' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWallShowersWithoutArm = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'WallShowersWithoutArm' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWallShowersWithoutArm = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'WallShowersWithoutArm' });
