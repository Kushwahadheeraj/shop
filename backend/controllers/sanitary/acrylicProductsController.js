...
exports.createacrylicProducts = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'acrylicProducts' };
...
exports.getAllacrylicProducts = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'acrylicProducts' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneacrylicProducts = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'acrylicProducts' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteacrylicProducts = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'acrylicProducts' });
