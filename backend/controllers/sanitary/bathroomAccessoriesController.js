...
exports.createBathroomAccessories = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'bathroomAccessories' };
...
exports.getAllBathroomAccessories = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'bathroomAccessories' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBathroomAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'bathroomAccessories' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBathroomAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'bathroomAccessories' });
