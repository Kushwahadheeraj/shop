...
exports.createClosets = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Closets' };
...
exports.getAllClosets = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Closets' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneClosets = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Closets' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteClosets = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Closets' });
