...
exports.createHardwareBathroomAccessories = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'HardwareBathroomAccessories' };
...
exports.getAllHardwareBathroomAccessories = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'HardwareBathroomAccessories' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHardwareBathroomAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'HardwareBathroomAccessories' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteHardwareBathroomAccessories = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'HardwareBathroomAccessories' });
