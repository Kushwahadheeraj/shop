...
exports.createHealthFaucet = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'HealthFaucet' };
...
exports.getAllHealthFaucet = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'HealthFaucet' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHealthFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'HealthFaucet' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteHealthFaucet = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'HealthFaucet' });
