...
exports.createJaquar = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Jaquar' };
...
exports.getAllJaquar = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Jaquar' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneJaquar = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Jaquar' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteJaquar = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Jaquar' });
