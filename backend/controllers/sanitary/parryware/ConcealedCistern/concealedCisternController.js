...
exports.createConcealedCistern = async (req, res) => {
  try {
    ...
    const product = new (require('../../models/SanitaryModels'))({ ...req.body, photos: photoUrls, category: 'parryware/ConcealedCistern/concealedCistern' });
    await product.save();
...
exports.updateConcealedCistern = async (req, res) => {
  try {
    ...
    const product = await require('../../models/SanitaryModels').findOneAndUpdate(
      { _id: req.params.id, category: 'parryware/ConcealedCistern/concealedCistern' },
      update,
      { new: true }
    );
    if (!product) return res.status(404).json({ error: 'Not found' });
