...
exports.createFlushCocks = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'FlushCocks' };
...
exports.getAllFlushCocks = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'FlushCocks' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFlushCocks = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'FlushCocks' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFlushCocks = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'FlushCocks' });
