...
exports.createFlushValve = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'FlushValve' };
...
exports.getAllFlushValve = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'FlushValve' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneFlushValve = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'FlushValve' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteFlushValve = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'FlushValve' });
