...
exports.createHealthFaucets = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'HealthFaucets' };
...
exports.getAllHealthFaucets = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'HealthFaucets' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneHealthFaucets = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'HealthFaucets' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteHealthFaucets = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'HealthFaucets' });
