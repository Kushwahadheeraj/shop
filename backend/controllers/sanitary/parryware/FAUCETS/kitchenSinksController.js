...
exports.createKitchenSinks = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'KitchenSinks' };
...
exports.getAllKitchenSinks = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'KitchenSinks' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneKitchenSinks = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'KitchenSinks' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteKitchenSinks = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'KitchenSinks' });
