...
exports.createPedestals = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'Pedestals' };
...
exports.getAllPedestals = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'Pedestals' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePedestals = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'Pedestals' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePedestals = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'Pedestals' });
