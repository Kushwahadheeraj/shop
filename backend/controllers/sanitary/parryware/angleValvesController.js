...
exports.createAngleValves = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'AngleValves' };
...
exports.getAllAngleValves = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'AngleValves' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneAngleValves = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'AngleValves' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteAngleValves = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'AngleValves' });
