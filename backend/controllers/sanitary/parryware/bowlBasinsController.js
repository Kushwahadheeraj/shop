...
exports.createBowlBasins = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'BowlBasins' };
...
exports.getAllBowlBasins = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'BowlBasins' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneBowlBasins = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'BowlBasins' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteBowlBasins = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'BowlBasins' });
