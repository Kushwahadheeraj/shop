...
exports.createEuropeanWaterCloset = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'EuropeanWaterCloset' };
...
exports.getAllEuropeanWaterCloset = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'EuropeanWaterCloset' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneEuropeanWaterCloset = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'EuropeanWaterCloset' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteEuropeanWaterCloset = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'EuropeanWaterCloset' });
