...
exports.createPolymerCisterns = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PolymerCisterns' };
...
exports.getAllPolymerCisterns = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'PolymerCisterns' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePolymerCisterns = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PolymerCisterns' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePolymerCisterns = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PolymerCisterns' });
