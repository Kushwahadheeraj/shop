...
exports.createPushPlates = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'PushPlates' };
...
exports.getAllPushPlates = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'PushPlates' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOnePushPlates = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'PushPlates' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deletePushPlates = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'PushPlates' });
