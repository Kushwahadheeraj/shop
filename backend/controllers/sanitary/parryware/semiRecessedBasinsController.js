...
exports.createSemiRecessedBasins = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'SemiRecessedBasins' };
...
exports.getAllSemiRecessedBasins = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'SemiRecessedBasins' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneSemiRecessedBasins = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'SemiRecessedBasins' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteSemiRecessedBasins = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'SemiRecessedBasins' });
