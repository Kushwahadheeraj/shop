...
exports.createShowerEnclosures = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ShowerEnclosures' };
...
exports.getAllShowerEnclosures = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ShowerEnclosures' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneShowerEnclosures = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ShowerEnclosures' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteShowerEnclosures = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ShowerEnclosures' });
