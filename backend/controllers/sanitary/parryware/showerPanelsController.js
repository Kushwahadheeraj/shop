...
exports.createShowerPanels = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'ShowerPanels' };
...
exports.getAllShowerPanels = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'ShowerPanels' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneShowerPanels = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'ShowerPanels' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteShowerPanels = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'ShowerPanels' });
