...
exports.createUtsavRange = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'UtsavRange' };
...
exports.getAllUtsavRange = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'UtsavRange' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneUtsavRange = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'UtsavRange' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteUtsavRange = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'UtsavRange' });
