...
exports.createWasteCoupling = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'WasteCoupling' };
...
exports.getAllWasteCoupling = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'WasteCoupling' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWasteCoupling = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'WasteCoupling' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWasteCoupling = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'WasteCoupling' });
