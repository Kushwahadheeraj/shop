...
exports.createWaterHeaters = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'WaterHeaters' };
...
exports.getAllWaterHeaters = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'WaterHeaters' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWaterHeaters = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'WaterHeaters' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWaterHeaters = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'WaterHeaters' });
