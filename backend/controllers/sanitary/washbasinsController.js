...
exports.createWashBasins = async (req, res) => {
  try {
    ...
    let productData = { ...req.body, photos: photoUrls, category: 'WashBasins' };
...
exports.getAllWashBasins = async (req, res) => {
  try {
    const products = await Sanitary.find({ category: 'WashBasins' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.getOneWashBasins = async (req, res) => {
  try {
    const product = await Sanitary.findOne({ _id: req.params.id, category: 'WashBasins' });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json(product);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
exports.deleteWashBasins = async (req, res) => {
  try {
    const product = await Sanitary.findOneAndDelete({ _id: req.params.id, category: 'WashBasins' });
