...
exports.createChisels = async (req, res) => {
  try {
    ...
    const product = new ToolsModel({ ...req.body, photos: photoUrls, category: 'chisels' });
    await product.save();
...
/**
 * Get all Chisels products.
 */
exports.getAllChisels = async (req, res) => {
  try {
    const products = await ToolsModel.find({ category: 'chisels' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
/**
 * Get a single Chisels product by ID.
 */
exports.getOneChisels = async (req, res) => {
  try {
    const product = await ToolsModel.findOne({ _id: req.params.id, category: 'chisels' });
...
/**
 * Delete a Chisels product by ID.
 */
exports.deleteChisels = async (req, res) => {
  try {
    const product = await ToolsModel.findOneAndDelete({ _id: req.params.id, category: 'chisels' });
    if (!product) return res.status(404).json({ error: 'Not found' });
