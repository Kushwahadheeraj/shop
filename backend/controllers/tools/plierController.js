...
exports.createPlier = async (req, res) => {
  try {
    ...
    const product = new ToolsModel({ ...req.body, photos: photoUrls, category: 'plier' });
    await product.save();
...
/**
 * Get all Plier products.
 */
exports.getAllPlier = async (req, res) => {
  try {
    const products = await ToolsModel.find({ category: 'plier' });
    res.json(products);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
/**
 * Get a single Plier product by ID.
 */
exports.getOnePlier = async (req, res) => {
  try {
    const product = await ToolsModel.findOne({ _id: req.params.id, category: 'plier' });
...
/**
 * Delete a Plier product by ID.
 */
exports.deletePlier = async (req, res) => {
  try {
    const product = await ToolsModel.findOneAndDelete({ _id: req.params.id, category: 'plier' });
    if (!product) return res.status(404).json({ error: 'Not found' });
