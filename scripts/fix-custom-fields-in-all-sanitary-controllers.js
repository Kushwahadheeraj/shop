...
// Function to process all controller files
function processControllers() {
  const items = fs.readdirSync(controllersDir);
  let processedCount = 0;
  
  for (const item of items) {
    const itemPath = path.join(controllersDir, item);
    const stat = fs.statSync(itemPath);
    
    if (stat.isFile() && item.endsWith('.js')) {
      if (addCustomFieldsProcessing(itemPath)) {
        processedCount++;
        console.log(`✅ Added Custom Fields processing to ${item}`);
      } else {
        console.log(`ℹ️  ${item} already has Custom Fields processing or no changes needed`);
      }
    }
  }
...
console.log('🔍 Starting to fix Custom Fields processing in all sanitary controllers...\n');
console.log('📋 This will add Custom Fields processing to ALL sanitary controllers\n');
console.log('🔄 Adding: Custom Fields parsing logic for both create and update functions\n');

// Process all controller files
const totalProcessed = processControllers();
...
console.log('🔍 Custom Fields data will now be saved to the database');
console.log('🚀 No more missing Custom Fields data!');
