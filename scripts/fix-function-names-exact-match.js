...
// Function to get the exact function name that router expects
function getRouterFunctionName(controllerName) {
  // Remove 'Controller.js' and get the base name
  let baseName = controllerName.replace('Controller.js', '');
  
  // Handle special cases for different naming patterns based on router
  if (baseName === 'acrylicProducts') {
    return 'acrylicProducts'; // lowercase 'a' as per router
  } else if (baseName === 'bathroomAccessories') {
    return 'BathroomAccessories'; // capital 'B' as per router
  } else if (baseName === 'closets') {
    return 'Closets'; // capital 'C' as per router
  } else if (baseName === 'faucets') {
    return 'Faucets'; // capital 'F' as per router
  } else if (baseName === 'hardwareBathroomAccessories') {
    return 'HardwareBathroomAccessories'; // capital 'H' as per router
  } else if (baseName === 'healthFaucet') {
    return 'HealthFaucet'; // capital 'H' as per router
  } else if (baseName === 'jaquar') {
    return 'Jaquar'; // capital 'J' as per router
  } else if (baseName === 'kitchenSinks') {
    return 'KitchenSinks'; // capital 'K' as per router
  } else if (baseName === 'showers') {
    return 'Showers'; // capital 'S' as per router
  } else if (baseName === 'taps') {
    return 'Taps'; // capital 'T' as per router
  } else if (baseName === 'washbasins') {
    return 'Washbasins'; // capital 'W' as per router
  }
  
  // For subdirectory controllers, use the base name as is (this is what router expects)
  return baseName;
}
...
function processController(controllerPath, controllerName, folderPath) {
  ...
  try {
    ...
    const functionName = getRouterFunctionName(controllerName);
...
console.log('🔍 Starting exact function name fix to match router...\n');
console.log('📋 This will update ALL function names to EXACTLY match what router expects\n');
...
console.log(`\n🎉 Function name fix completed! Total controllers processed: ${totalProcessed}`);
console.log('✅ All sanitary controllers now have function names that EXACTLY match the router!');
