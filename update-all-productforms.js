...
// Folders to exclude
const EXCLUDE = ['Adhesives', 'Brush', 'Cements', 'Cleaning', 'Dry'];
const PRODUCT_ADD_PATH = path.join(__dirname, 'app', 'Dashboard', 'ProductAdd');

function getAllProductFormFiles(dir) {
  let results = [];
  const list = fs.readdirSync(dir, { withFileTypes: true });
  for (const file of list) {
    if (file.isDirectory()) {
      if (!EXCLUDE.includes(file.name)) {
        results = results.concat(getAllProductFormFiles(path.join(dir, file.name)));
      }
...
function getApiEndpoint(filePath) {
  ...
  const idx = parts.lastIndexOf('ProductAdd');
  if (idx !== -1 && parts.length > idx + 2) {
    const main = parts[idx + 1];
    const sub = parts[idx + 2];
    
    // For Electrical folder, use the exact folder names as specified
    if (main === 'Electrical') {
      return `electrical/${sub}/create`;
    }
    
    // For other folders, use lowercase main folder and original sub folder
    return `${main.toLowerCase()}/${sub}/create`;
  }
  return 'unknown/create';
}

const template = (category, apiEndpoint, folderName) => `"use client";
...
export default function ProductForm() {
  ...
  const handleSubmit = async e => {
    ...
    const res = await fetch(\`\${API_BASE_URL}/locks/${folderName}/create\`, { method: 'POST', body: data });
...
function isUpToDate(file, category, apiEndpoint, folderName) {
  try {
    const content = fs.readFileSync(file, 'utf8');
    // Check for key template markers
    return (
      content.includes('import API_BASE_URL from "@/lib/apiConfig";') &&
      content.includes('locks/' + folderName + '/create') &&
...
function main() {
  const files = getAllProductFormFiles(PRODUCT_ADD_PATH);
  // Filter only Locks folder files
  const locksFiles = files.filter(file => file.includes('\\Locks\\') || file.includes('/Locks/'));
  
  let updated = 0;
  let skipped = [];
  let alreadyUpToDate = 0;
  
  console.log(`Found ${locksFiles.length} Locks ProductForm.jsx files`);
  
  for (const file of locksFiles) {
    const category = getCategoryFromPath(file);
    const apiEndpoint = getApiEndpoint(file);
    const folderName = path.basename(path.dirname(file));
    const parentFolder = path.basename(path.dirname(path.dirname(file)));
    
    // Get the correct API endpoint for Locks folder
    let correctApiEndpoint = '';
    if (parentFolder === 'Locks') {
      // Map folder names to API endpoints
      const folderMapping = {
      ...
      correctApiEndpoint = folderMapping[folderName] || folderName.toLowerCase();
    }
    
    try {
      if (isUpToDate(file, category, apiEndpoint, folderName)) {
        alreadyUpToDate++;
        continue;
      }
      fs.writeFileSync(file, template(category, apiEndpoint, correctApiEndpoint), 'utf8');
